...
2. Open the Enterprise Lab page and list deterministic scenarios through `GET /api/lab/scenarios`. Run a small lab comparison through `POST /api/lab/runs`, then point to the scorecard, guardrail reasons, rollback reasons, policy audit events, lab metrics, and `lab evidence only / not production activation` recommendation.
...
Enterprise Lab observability evidence:
