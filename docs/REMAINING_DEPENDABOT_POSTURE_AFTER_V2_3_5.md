...
- OpenJFX effective-model warning: fixed by validating and retaining the same-major JavaFX patch from `17.0.10` to `17.0.19`. The earlier workstation PKIX blocker was resolved locally before validation; package, full test, verify, and smoke checks passed without the OpenJFX effective-model warning.
