...
- [`ENTERPRISE_COCKPIT_AUTH_PLAN.md`](docs/ENTERPRISE_COCKPIT_AUTH_PLAN.md) documents the cockpit auth product mismatch, endpoint exposure matrix, and planned Swagger/OpenAPI gating path.
...
If `LOADBALANCERPRO_API_KEY` is missing or blank, protected prod-profile API and proxy-surface requests fail closed with HTTP 401. `/api/health`, Actuator health/info, and OpenAPI docs remain public for local validation and portfolio review.
...
The profile does not require AWS credentials just to start the API, does not mutate AWS resources by default, and still protects API mutation/LASE observability endpoints plus proxy forwarding/status surfaces with `X-API-Key`. Treat it as a cloud-safety validation lane, not production mode.
