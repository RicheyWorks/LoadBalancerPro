...
5. Operator run-profile dry-run: run `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\smoke\operator-run-profiles-smoke.ps1 -DryRun`.
6. Postman safe smoke dry-run: run `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\smoke\postman-enterprise-lab-safe-smoke.ps1 -DryRun`.
7. CI evidence: inspect the latest successful CI, CodeQL, Dependency Review, `jacoco-coverage-report`, `packaged-artifact-smoke`, and `loadbalancerpro-sbom` artifacts.
...
8. Read [`PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md`](PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md) for the implemented approval gate and remaining requirements before any broader private-network traffic is implemented.
9. Use [`REAL_BACKEND_PROXY_EXAMPLES.md`](REAL_BACKEND_PROXY_EXAMPLES.md) to adapt the same verification model to local/private HTTP services.
10. Read the auth/TLS boundary in [`REVERSE_PROXY_MODE.md`](REVERSE_PROXY_MODE.md#auth-and-tls-boundary) before exposing proxy mode beyond loopback.
11. Verify status through `/proxy-status.html` and `/api/proxy/status`.
