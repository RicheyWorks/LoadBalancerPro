...
Container/default deployment mode is protected by the prod API-key profile. The checked-in Dockerfile defaults `SPRING_PROFILES_ACTIVE=prod`, and operators provide `LOADBALANCERPRO_API_KEY` at run time for `/api/**`, proxy, OpenAPI, and Swagger review. The explicit public prod/cloud-sandbox API-key exceptions are `GET /api/health` and unauthenticated `OPTIONS` preflight requests.
...
```bash
mvn spring-boot:run "-Dspring-boot.run.arguments=--server.address=127.0.0.1 --server.port=8080 --spring.profiles.active=local"
