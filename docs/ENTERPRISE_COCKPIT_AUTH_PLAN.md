...
- This is Enterprise Lab cockpit hardening guidance, not production IAM certification.
...
- Current-state update (2026-07-29): API-key enforcement is now profile-independent, missing-key API-key startup is refused, explicit `auth.mode=none` is warned and local-only, default Actuator exposure is health/info only, and OAuth2 uses explicit read/operator/admin classification with an unclassified-API deny fallback.

## Current Model

The unqualified default selects `loadbalancerpro.auth.mode=api-key` and refuses startup without a key. The explicit `local` profile selects `auth.mode=none`, emits a prominent authentication-disabled warning, and keeps the browser/API demos available for loopback-only development and CI smoke tests. All checked-in defaults expose Actuator health/info only.

Whenever API-key mode is selected, `ProdApiKeyFilter` protects non-OPTIONS `/api/**`, `/proxy` or `/proxy/**`, `/v3/api-docs`, `/v3/api-docs/**`, `/swagger-ui.html`, and `/swagger-ui/**` with `X-API-Key`, regardless of profile. Missing/blank configuration prevents startup; a missing/wrong request header returns 401 after valid configuration. The explicit public API exceptions are `GET /api/health` and unauthenticated `OPTIONS`. Static browser assets remain public, but their protected API calls still require the active mode's credentials.

OAuth2 mode is a real Spring Security resource-server foundation, not just documentation. When `loadbalancerpro.auth.mode=oauth2` is active, startup requires either an issuer URI or JWK set URI, JWTs are decoded through Spring Security, and application roles are extracted from dedicated `roles`, `role`, `authorities`, and `realm_access.roles` claims. Ordinary OAuth2 `scope` and `scp` claims are not app roles and do not become `ROLE_operator` or `ROLE_admin`. `/api/health` and CORS preflight remain public. Every current API prefix has an explicit read/operator/admin rule, durable retention/compaction requires admin, and unclassified `/api/**` routes are denied. `/v3/api-docs` and Swagger UI are gated by default unless `loadbalancerpro.auth.docs-public=true`. Static browser pages are not an implemented OAuth2 cockpit workflow today; OAuth2 login/session UX is not present in this sprint.

For concrete enterprise IdP examples, use [`IDP_CLAIM_MAPPING_EXAMPLES.md`](IDP_CLAIM_MAPPING_EXAMPLES.md). It shows dedicated role-claim examples for `operator` and `admin`, scope-only tokens that fail closed, and the local/mock `OAuth2AuthorizationTest` path that requires no real tenant IDs, client secrets, JWKs, or live IdP calls.

The static cockpit and lab pages are browser-only assets. `/load-balancing-cockpit.html` calls same-origin `/api/health`, Actuator readiness, `POST /api/allocate/capacity-aware`, `POST /api/allocate/evaluate`, and `POST /api/routing/compare`. The cockpit now includes an operator API-key control for API-key mode: it stores the entered value in JavaScript memory only, clears it on refresh/navigation, does not use `localStorage` or `sessionStorage`, does not put keys in URLs or logs, and adds `X-API-Key` to protected `/api/**` calls after the operator configures it. Copyable curl snippets use `<API_KEY>` instead of the entered value. `/routing-demo.html` is the Enterprise Lab routing cockpit legacy route and calls readiness and `POST /api/routing/compare`. `/evidence-training-demo.html` calls read-only onboarding/template/example/scorecard routes and `POST /api/evidence-training/scorecards/grade`. Those other local lab pages still send JSON or Accept headers only; they do not collect, store, or send `X-API-Key` or `Authorization` credentials, so they are local/default browser pages unless separately enhanced for prod/cloud-sandbox API-key review.

Actuator exposure is health/info only across the checked-in default, local, prod, and cloud-sandbox configuration, with Prometheus export disabled. Metrics and Prometheus must stay private or disabled unless a future sprint explicitly gates them behind trusted deployment controls.

## Product Mismatch

- Backend `/api/**` routes require `X-API-Key` whenever API-key mode is selected except `GET /api/health` and unauthenticated `OPTIONS`, and the cockpit has a memory-only operator token path for those calls. Other static lab pages are usable without credentials only in explicit warned none mode.
...
- Public `/v3/api-docs` and Swagger UI in API-key mode were the previous behavior. They are now gated by `X-API-Key` in every API-key profile; explicit local none mode preserves loopback developer usability.
...
The table below preserves the cockpit-hardening sprint's design matrix. Where its local/default or profile-specific
phrasing conflicts with the current-state update above, the current fail-closed/default and explicit-none contract
controls.
...
| `/routing-demo.html` | Enterprise Lab routing cockpit legacy route. | Currently public static page; `POST /api/routing/compare` requires `X-API-Key`. | Authenticated operator or local/default only. | The page can show form state but prod-like compare calls fail without auth. | Keep as local/operator lab review unless gated. |
...
- Validate IdP claim shape against [`IDP_CLAIM_MAPPING_EXAMPLES.md`](IDP_CLAIM_MAPPING_EXAMPLES.md) before reviewer demos; scope-only tokens authenticate but do not grant app roles.
...
| Postman/curl-only prod operations | Avoids browser secret handling. | Lower operator ergonomics; cockpit remains browser-local unless gated. | Acceptable interim posture for prod-like validation. |
