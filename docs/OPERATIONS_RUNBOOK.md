...
For the Enterprise Lab workflow, open `http://localhost:8080/enterprise-lab.html` in local/demo mode, call `GET /api/lab/scenarios` to list deterministic adaptive-routing scenarios, and call `POST /api/lab/runs` to compare baseline, shadow, and explicit opt-in influence outcomes with scorecards. Runtime run storage is process-local, in-memory, and bounded by retained-run and scenario-count limits. For ignored reviewer evidence, run `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\smoke\enterprise-lab-workflow.ps1 -Package`; it writes scenario catalog JSON, run JSON, Markdown scorecard summary, and metadata only under `target/enterprise-lab-runs/`. The workflow does not call `CloudManager`, mutate cloud state, use external network services, create releases/tags/assets, or publish containers.
...
For Enterprise Lab APIs in prod or cloud-sandbox API-key mode, include `X-API-Key` for `/api/lab/scenarios`, `/api/lab/runs`, and retained-run lookups. In OAuth2 mode, scenario catalog reads use configured read roles and run creation/retrieval uses the allocation role. Local/default mode remains intentionally convenient and should stay loopback/private.
