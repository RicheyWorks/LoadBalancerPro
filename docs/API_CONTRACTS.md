...
`POST /api/routing/compare` remains read-only and recommendation-only. Its top-level response can expose additive reviewer metadata `decisionReplayEvidenceReviewerClosureRollup` and `decisionReplayEvidenceReviewerClosureChecklist`, and its routing result objects can expose additive lab explainability fields: `decisionVector`, `dominantFactorAnalysis`, `decisionDeltaAnalysis`, `decisionReplaySnapshot`, `decisionReplayReconstructionTrace`, `decisionReplayCapsule`, `decisionReplayReadinessChecklist`, `decisionReplayEvidenceSourceMap`, `decisionReplayEvidenceBoundarySummary`, `decisionReplayEvidenceFieldInventory`, `decisionReplayEvidenceNullSafetySummary`, `decisionReplayEvidenceStatusRollup`, `decisionReplayEvidenceLaneNavigationSummary`, `decisionReplayEvidenceLaneDependencyMap`, `decisionReplayEvidenceLaneReferenceIndex`, `decisionReplayEvidenceLaneDependencySummary`, `decisionReplayEvidenceLaneConsistencySummary`, `decisionReplayEvidenceReviewerSnapshot`, `decisionReplayEvidenceReviewerGuidance`, `decisionReplayEvidenceReviewerHandoffSummary`, and `decisionReplayEvidenceReviewerClosureSummary`. The reviewer closure rollup and checklist are derived only from already-built `results[].decisionReplayEvidenceReviewerClosureSummary` data and `decisionReplayEvidenceReviewerClosureRollup` data; the replay snapshot, reconstruction trace, replay capsule, replay readiness checklist, evidence source map, evidence boundary summary, evidence field inventory, evidence null-safety summary, evidence status rollup, evidence lane navigation summary, evidence lane dependency map, evidence lane reference index, evidence lane dependency summary, evidence lane consistency summary, evidence reviewer snapshot, evidence reviewer guidance, evidence reviewer handoff summary, and evidence reviewer closure summary are derived from already-built compare evidence and stable analysis/status/navigation/dependency/reviewer metadata; they do not execute replay, perform what-if mutation, persist audit logs, traces, capsules, checklist state, source-map data, boundary-summary data, field-inventory data, null-safety-summary data, status-rollup data, lane-navigation data, lane-dependency data, lane-reference-index data, lane-dependency-summary data, lane-consistency-summary data, reviewer-snapshot data, reviewer-guidance data, reviewer-handoff data, reviewer-closure data, reviewer-closure-rollup data, or reviewer-closure-checklist data, generate a new source-map, boundary-summary, field-inventory, null-safety, status-rollup, lane-navigation, lane-dependency, lane-reference-index, lane-dependency-summary, lane-consistency-summary, reviewer-snapshot, reviewer-guidance, reviewer-handoff, reviewer-closure, reviewer-closure-rollup, or reviewer-closure-checklist fingerprint, recompute scores, retune weights, or add upload/share/download routes or server-side export/PDF/ZIP generation.

The same top-level response can expose additive reviewer metadata `decisionReplayEvidenceReviewerClosurePacket`. It is derived only from `results[].decisionReplayEvidenceReviewerClosureSummary`, `decisionReplayEvidenceReviewerClosureRollup`, and `decisionReplayEvidenceReviewerClosureChecklist`; it is not an export/share/download packet, does not persist packet data, does not generate a new fingerprint, does not recompute scores, does not execute replay, and does not add server-side export/PDF/ZIP generation.
