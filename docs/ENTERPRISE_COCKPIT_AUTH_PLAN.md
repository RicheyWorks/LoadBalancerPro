...
- This is a plan/audit document.
- No runtime behavior changed in this sprint.
- This is enterprise-demo hardening guidance, not production IAM certification.
- This document describes target operator authentication and OpenAPI gating behavior. It does not implement OAuth2 login, browser token storage, reverse-proxy identity trust, or Swagger gating.
...
Prod and cloud-sandbox API-key mode use `ProdApiKeyFilter` when the active profile is `prod` or `cloud-sandbox` and `loadbalancerpro.auth.mode=api-key`. The filter protects non-OPTIONS `POST`, `PUT`, and `PATCH` requests under `/api/**`, `GET /api/lase/**`, `GET /api/proxy/status`, and `/proxy` or `/proxy/**` with the `X-API-Key` header. A missing configured API key fails closed for those protected requests. `/api/health`, static browser pages, `/v3/api-docs`, and Swagger UI remain public by current API-key-mode behavior. Prod and cloud-sandbox profiles expose Actuator health/info only by default and keep metrics/prometheus disabled by profile configuration.

OAuth2 mode is a real Spring Security resource-server foundation, not just documentation. When `loadbalancerpro.auth.mode=oauth2` is active, startup requires either an issuer URI or JWK set URI, JWTs are decoded through Spring Security, and roles are extracted from common role, authority, scope, and realm-access claims. `/api/health` and CORS preflight remain public. Allocation, routing, proxy, and LASE shadow routes require configured roles. `/v3/api-docs` and Swagger UI are gated by default unless `loadbalancerpro.auth.docs-public=true`. Static browser pages are not an implemented OAuth2 cockpit workflow today; OAuth2 login/session UX is not present in this sprint.

The static cockpit and demo pages are browser-only assets. `/load-balancing-cockpit.html` calls same-origin `/api/health`, Actuator readiness, `POST /api/allocate/capacity-aware`, `POST /api/allocate/evaluate`, and `POST /api/routing/compare`. `/routing-demo.html` calls readiness and `POST /api/routing/compare`. `/evidence-training-demo.html` calls read-only onboarding/template/example/scorecard routes and `POST /api/evidence-training/scorecards/grade`. Current fetch calls send JSON or Accept headers only; they do not collect, store, or send `X-API-Key` or `Authorization` credentials.
...
- Backend protected POSTs can require `X-API-Key` in prod/cloud-sandbox API-key mode, but the browser cockpit currently behaves like a local/demo UI and does not supply operator auth.
...
- Public `/v3/api-docs` and Swagger UI in prod-like API-key mode are current behavior. That should be an intentional local/private review choice or should be gated/disabled for enterprise deployments.
...
| `/load-balancing-cockpit.html` | Public local cockpit. | Currently public static page, but protected POST calls fail without `X-API-Key`. Target: disable/gate in prod or add a safe operator token flow. | Authenticated operator cockpit after login/session or disabled until OAuth2 UI exists. | Main mismatch: UI renders but cannot operate protected prod APIs unless auth is provided. | Current page sends JSON headers only and keeps state in memory. |
...
| `/evidence-training-demo.html` | Public local evidence training UI. | Currently public static page; grading POST requires `X-API-Key` in prod/cloud-sandbox API-key mode. | Authenticated viewer for discovery and operator for grading, or disabled until OAuth2 UI exists. | Current demo can browse packaged read-only content; protected grade call needs auth in prod-like mode. | No credential fields or browser storage today. |
...
| `/v3/api-docs` | Public local generated OpenAPI. | Currently public in API-key mode. Target: gate or disable by default for prod/cloud-sandbox API-key mode. | Require authenticated viewer/operator/developer role unless `docs-public=true` is intentionally set for demo/private review. | Swagger-generated schema should not be assumed public in enterprise mode. | Existing test coverage documents current public API-key behavior; changing it needs focused tests. |
| Swagger UI (`/swagger-ui.html`, `/swagger-ui/**`) | Public local UI. | Currently public in API-key mode. Target: gate or disable by default for prod/cloud-sandbox API-key mode. | Require authenticated viewer/operator/developer role unless explicitly made public for demo/private review. | Browser docs should align with the same auth mode as the cockpit. | Gate UI and JSON docs together. |
...
- The cockpit should either be disabled/gated in prod or support a safe operator token entry/session pattern.
- Do not persist API keys in `localStorage` unless that risk is explicitly accepted and documented.
- Prefer memory-only token entry for a lightweight app-native path or reverse-proxy injected auth for serious deployments.
...
- prod-api-key: gate or disable `/v3/api-docs`, `/v3/api-docs/**`, `/swagger-ui.html`, and `/swagger-ui/**` by default. If a deployment keeps docs public, it should be an explicit local/private review override.
...
- Phase 1: docs + route matrix. This sprint adds the audit plan and docs/static validation only.
- Phase 2: gate Swagger in prod API-key mode.
- Phase 3: add cockpit memory-only operator token support for API-key mode.
...
- No token or API key is stored in `localStorage` unless explicitly chosen and documented.
...
- No cockpit token storage implementation in this sprint.
...
Run one aggressive implementation sprint: gate `/v3/api-docs` and Swagger UI by default in prod/cloud-sandbox API-key mode, keep local/demo docs public, preserve existing prod API-key protections, and add focused tests proving protected APIs, docs routes, Swagger UI, and Actuator metrics/prometheus exposure all keep the intended profile-specific behavior.
