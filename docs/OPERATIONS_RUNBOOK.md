...
For automated reviewer evidence without starting external services, run `mvn -Dtest=LocalOnlyRealBackendProxyValidationTest test`. This source-visible JUnit path starts only JDK loopback `HttpServer` fixtures on Java-assigned ephemeral ports, proves real forwarding status/body/header behavior, and verifies a configured-unavailable backend fails closed. It does not add native tools, port scanning, persistence, service installation, scheduled tasks, credential storage, or external network calls.
