...
| Lightweight reverse proxy mode | `ReverseProxyDisabledTest`, `ReverseProxyControllerTest`, `ReverseProxyHealthAwareTest`, `ReverseProxyHealthMetricsTest`, `ReverseProxyFailureTest`, `ReverseProxyRetrySafetyTest`, `ReverseProxyRetryCooldownTest`, `ReverseProxyStrategyDemoLabTest`, `ProxyOperatorStatusPageTest`, `ProxyDemoStackUsabilityTest`, `ProxyDemoFixtureLauncherTest` | Uses in-process loopback upstream fixtures and static safety checks to prove real local HTTP forwarding, query/body preservation, configured and active health-aware skipping, selected-upstream and strategy headers for `ROUND_ROBIN`, `WEIGHTED_ROUND_ROBIN`, and health-aware failover demos, read-only status output, forwarding/failure/retry/cooldown/status-class counters, bounded GET retry behavior, non-idempotent no-retry defaults, cooldown skip/recovery, upstream failure handling, static status UI safety, checked-in demo profile/script/docs safety, Java fixture launcher parsing/output/health/echo behavior, default proxy disabled behavior, and no `CloudManager` construction. |
