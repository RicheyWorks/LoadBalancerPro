...
5. Private-network live gate status: with the app running, inspect `/api/proxy/status.privateNetworkLiveValidation`; it reports flags, classifier decisions, reason codes, and `trafficExecuted=false` without calling the executor.
...
9. Read [`PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md`](PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md) for the implemented approval gate and remaining requirements before any broader private-network traffic is implemented.
10. Use [`REAL_BACKEND_PROXY_EXAMPLES.md`](REAL_BACKEND_PROXY_EXAMPLES.md) to adapt the same verification model to local/private HTTP services.
11. Read the auth/TLS boundary in [`REVERSE_PROXY_MODE.md`](REVERSE_PROXY_MODE.md#auth-and-tls-boundary) before exposing proxy mode beyond loopback.
12. Verify status through `/proxy-status.html` and `/api/proxy/status`.
