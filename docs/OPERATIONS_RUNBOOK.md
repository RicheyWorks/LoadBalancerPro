...
For the Enterprise Lab workflow, open `http://localhost:8080/enterprise-lab.html` in local/demo mode, call `GET /api/lab/scenarios` to list deterministic adaptive-routing scenarios, call `POST /api/lab/runs` to compare baseline, shadow, recommend, and active-experiment outcomes with scorecards, and call `GET /api/lab/policy` plus `GET /api/lab/audit-events` to inspect the controlled policy status and bounded audit trail. Runtime run storage and audit events are process-local, in-memory, and bounded by retained-run, audit-event, and scenario-count limits. For ignored reviewer evidence, run `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\smoke\enterprise-lab-workflow.ps1 -Package`; it writes scenario catalog JSON, run JSON, Markdown scorecard summary, and metadata only under `target/enterprise-lab-runs/`. For controlled policy evidence, run `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\smoke\controlled-adaptive-routing-policy.ps1 -Package`; it writes JSON and Markdown policy evidence only under `target/controlled-adaptive-routing/`. These workflows do not call `CloudManager`, mutate cloud state, use external network services, create releases/tags/assets, or publish containers.
...
For Enterprise Lab APIs in prod or cloud-sandbox API-key mode, include `X-API-Key` for `/api/lab/scenarios`, `/api/lab/runs`, retained-run lookups, `/api/lab/policy`, and `/api/lab/audit-events`. In OAuth2 mode, scenario catalog reads use configured read roles and run creation/retrieval plus policy/audit status use the allocation role. Local/default mode remains intentionally convenient and should stay loopback/private.
