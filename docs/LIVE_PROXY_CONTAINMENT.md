...
The first live-validation traffic primitive is JUnit-only loopback proof. [`PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md`](PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md) documents `loadbalancerpro.proxy.private-network-live-validation.enabled=false` and `loadbalancerpro.proxy.private-network-live-validation.operator-approved=false` as default-off properties. `PrivateNetworkLiveValidationGate` requires both to be true alongside `loadbalancerpro.proxy.private-network-validation.enabled=true`, enabled proxy mode, and classifier-approved literal backend URLs before it returns an allowed decision. `GET /api/proxy/status` now exposes `privateNetworkLiveValidation` as a report-only gate view with flags, classifier decisions, reason codes, and `trafficExecuted=false`; it does not call the executor or send validation traffic. `PrivateNetworkLiveValidationExecutor` then requires an allowed gate result before delegating one bounded request to an injected transport. `PrivateNetworkLiveValidationExecutorTest` supplies a test-only transport to a JDK `HttpServer` bound to loopback with a Java-assigned ephemeral port and writes redacted ignored evidence to `target/proxy-evidence/private-network-live-loopback-validation.md` and `target/proxy-evidence/private-network-live-loopback-validation.json`. The executor now fails closed for absolute, scheme-relative, query-string, fragment, traversal, encoded traversal, encoded control-character, raw control-character, blank, null, and backslash request paths before transport; propagates only allowlisted deterministic validation headers; captures only allowlisted response summary headers; and reports loopback redirects without following public `Location` targets. The executor is not wired into app startup, Postman, smoke scripts, or proxy routing. Broader private-LAN live validation remains separately gated and unimplemented; there is still no DNS resolution, reachability probe, discovery, port scan, Postman/smoke live execution, or routing behavior change.

`POST /api/proxy/private-network-live-validation` is the current operator-trigger command contract only. It is protected by the existing prod API-key/OAuth2 boundary, validates request shape and offline gate state, returns `trafficExecuted=false` and `evidenceWritten=false`, and always reports that traffic execution is not wired in this release. Its evidence contract fields are plan-only: `plannedEvidenceDirectory="target/proxy-evidence/"`, `plannedEvidenceMarkdown="private-network-live-validation.md"`, `plannedEvidenceJson="private-network-live-validation.json"`, and `auditTrail.auditTrailWritten=false`. It does not call `PrivateNetworkLiveValidationExecutor`, write evidence or audit files, or send private-LAN/public traffic.
