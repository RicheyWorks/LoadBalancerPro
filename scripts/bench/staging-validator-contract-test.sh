...
assert_rejected identical-digests '.artifact.candidate.imageDigest = .artifact.prior.imageDigest'
assert_rejected single-zone '.deployment.minimumZones = 1'
assert_rejected loose-zone-skew '.deployment.maximumZoneSkew = 2'
assert_rejected missing-rollback-hook 'del(.hooks["rollback-prior"])'
assert_rejected short-rollout-traffic '.scenarios.candidateRollout.durationSeconds = 120'
assert_rejected inverted-memory-budget '.deployment.resources.memoryRequestMiB = 1024'
assert_rejected missing-required-signal '.deployment.observability.requiredSignals -= ["lbp_proxy_requests_total"]'

printf 'Staging validator accepted the example boundary and rejected 16 unsafe mutations.\n'
