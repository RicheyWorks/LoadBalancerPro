...
5. Private-network live gate status and command contract: with the app running, inspect `/api/proxy/status.privateNetworkLiveValidation` and, if needed, POST a local operator command to `/api/proxy/private-network-live-validation`; both report `trafficExecuted=false` without calling the executor.
...
- the private-network live validation command contract is protected, non-executing, writes no evidence, and returns `trafficExecuted=false`;
...
9. POST `/api/proxy/private-network-live-validation` when you want the protected command contract shape; it validates `requestPath`, evaluates the offline gate, and returns `INVALID_REQUEST`, `BLOCKED_BY_GATE`, or `NOT_IMPLEMENTED` with `trafficExecuted=false` and no evidence write.
