...
This document is a docs/test-only future Bruno API check collection design spec. It converts the Bruno lane from the local-lab boundary plan into concrete future collection shapes without adding collection files.

No Bruno collection files are added in this PR. No live API or production API checks are added. No external calls are added. No Docker Compose files are added in this PR. No actual tool execution occurs in this PR.
...
This design does not add Bruno collections, k6 scripts, Toxiproxy config, Docker/Docker Compose files, scripts, CI jobs, Maven dependencies, production endpoints, production listeners, external calls, live API checks, replay execution, evidence/report generation, storage/export behavior, load testing, stress testing, benchmarking, throughput evidence, or p95/p99 evidence.
...
| collection naming and folder structure as design only | proposed names and grouping for later Bruno files | design-only; no Bruno collection files are added |
