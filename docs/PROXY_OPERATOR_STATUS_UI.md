...
For the complete local demo-stack path, use [`PROXY_DEMO_STACK.md`](PROXY_DEMO_STACK.md). It points Windows and Unix users to the Java fixture launcher, helper scripts, checked-in `proxy-demo-*` profiles, startup commands, curl verification, status-page review, and cleanup steps. Launcher-specific fixture behavior is documented in [`PROXY_DEMO_FIXTURE_LAUNCHER.md`](PROXY_DEMO_FIXTURE_LAUNCHER.md).
