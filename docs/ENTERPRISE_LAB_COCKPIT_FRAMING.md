...
[`ENTERPRISE_LAB_DECISION_VECTOR.md`](ENTERPRISE_LAB_DECISION_VECTOR.md) defines the structured explanation contract for one controlled lab routing decision. It turns final selected-backend outcomes into structured decision evidence: selected strategy, selected backend/server, candidate backend list, selected candidate vector, non-selected candidate vectors, visible candidate signals, known signals, unknown or unexposed signals, degradation/fallback indicators, reason categories, selected-vs-alternative notes, exact scoring availability or absence, factor contribution availability or absence, dominant factor analysis derived from returned contribution data, decision delta analysis derived from returned score and contribution data, Decision Replay Snapshot evidence derived from already-built compare response fields, Decision Replay Reconstruction Trace evidence steps derived from already-built compare response fields, Decision Replay Capsule canonical evidence packaging derived from already-built compare response fields, Decision Replay Readiness Checklist status derived from already-built evidence lane statuses and linked fingerprints, replay readiness, lab proof boundary, and production not-proven boundary.
