...
| Adaptive-routing lab evidence | `POST /api/allocate/evaluate` exposes optional `laseShadow` and `lasePolicy` summaries. The policy gate supports `off`, `shadow`, `recommend`, and `active-experiment`, defaults safe/off, keeps `shadow` and `recommend` non-mutating, and allows active-experiment changes only when explicit config and health/eligibility/capacity/freshness/conflict/rollback/bounded-context gates pass. The Enterprise Lab workflow adds `GET /api/lab/scenarios`, `POST /api/lab/runs`, `GET /api/lab/policy`, `GET /api/lab/audit-events`, bounded in-memory run/audit retrieval, scorecards, `/enterprise-lab.html`, ignored `target/enterprise-lab-runs/` evidence export, and controlled policy evidence under `target/controlled-adaptive-routing/`. Default runtime allocation behavior remains unchanged. | [`API_CONTRACTS.md`](API_CONTRACTS.md), [`CONTROLLED_ACTIVE_LASE_POLICY_GATE.md`](CONTROLLED_ACTIVE_LASE_POLICY_GATE.md), [`DEMO_WALKTHROUGH.md`](DEMO_WALKTHROUGH.md), [`SRE_DEMO_HIGHLIGHTS.md`](SRE_DEMO_HIGHLIGHTS.md), `LaseAllocationShadowIntegrationTest`, `AdaptiveRoutingPolicyEngineTest`, `EnterpriseLabControllerTest` |
