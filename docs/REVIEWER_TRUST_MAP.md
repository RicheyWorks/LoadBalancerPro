...
- live private-network traffic execution is not implemented yet; the current default-off gate is offline only and remains bounded by [`PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md`](PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md).
...
| Is the future private-network proxy profile bounded before live execution? | Design plan, config-only validation gate, dry-run evidence, offline live gate skeleton, and static guard tests | [`PRIVATE_NETWORK_PROXY_PROFILE_PLAN.md`](PRIVATE_NETWORK_PROXY_PROFILE_PLAN.md), [`PRIVATE_NETWORK_PROXY_DRY_RUN.md`](PRIVATE_NETWORK_PROXY_DRY_RUN.md), [`PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md`](PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md) | Explicit operator-provided backend URLs, local/private-network allowlist model, `PrivateNetworkProxyDryRunEvidenceTest`, `PrivateNetworkLiveValidationGate`, `target/proxy-evidence/private-network-validation-dry-run.md`, `target/proxy-evidence/private-network-validation-dry-run.json` | The future profile path has opt-in startup/reload URL validation, dry-run-only classifier evidence, implemented default-off live gate properties, explicit operator approval gating, and forbids scanning, discovery, persistence, service installation, scheduled tasks, native tooling, secret persistence, public internet validation, and `release-downloads/` mutation before any live execution | Private-network live traffic execution, public exposure approval, or resolver policy approval |
...
7. Read [`PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md`](PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md) for the implemented offline approval gate and remaining requirements before any private-network traffic is implemented.
...
Read [`PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md`](PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md) before proposing any private-network live validation. `PrivateNetworkLiveValidationGate` is an implemented offline decision helper, not a traffic executor. It requires `loadbalancerpro.proxy.private-network-live-validation.enabled=true`, explicit operator approval through `loadbalancerpro.proxy.private-network-live-validation.operator-approved=true`, config validation, enabled proxy mode, operator-provided literal backend URLs, and `ProxyBackendUrlClassifier` approval before it can return allowed. The default properties are `loadbalancerpro.proxy.private-network-live-validation.enabled=false` and `loadbalancerpro.proxy.private-network-live-validation.operator-approved=false`; live private-network traffic execution is not implemented yet and still requires a separate approved task with bounded timeout behavior, redacted ignored `target/` evidence, API-key/OAuth2 boundary proof, no DNS, no discovery, no scanning, no persistence, no service installation, no scheduled tasks, no native tooling, no secret persistence, and fail-closed startup/reload behavior.
