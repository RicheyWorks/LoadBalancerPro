...
Replay snapshots, reconstruction traces, replay capsules, readiness checklists, evidence source maps, boundary summaries, field inventories, null-safety summaries, and status rollups are read-only lab evidence. They do not execute replay, perform what-if mutation, persist traces, capsules, checklists, source maps, boundary summaries, field inventories, null-safety summaries, status rollups, or audit logs server-side, recompute scores, retune weights, export/download/share those evidence objects, generate a field-inventory, null-safety, or status-rollup fingerprint, or add upload/share/download/export/PDF/ZIP functionality.
