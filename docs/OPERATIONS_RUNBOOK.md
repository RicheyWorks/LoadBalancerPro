...
For the future operator-triggered live validation command contract, use `POST /api/proxy/private-network-live-validation` only to validate request shape and the offline gate. The request accepts a simple `requestPath`, optional `evidenceRequested`, and optional operator acknowledgement. It returns `INVALID_REQUEST`, `BLOCKED_BY_GATE`, or `NOT_IMPLEMENTED` with `accepted=false`, `executable=false`, `trafficExecuted=false`, and `evidenceWritten=false`; traffic execution is not wired in this release. In prod/cloud-sandbox API-key mode, include `X-API-Key`; in OAuth2 mode, use the configured allocation role. This command does not call the executor, send private-LAN/public traffic, write evidence, or echo secrets.
