...
PR #485 remote-inspection tooling failure: after exact-head push/PR CI and CodeQL were visibly active in the PR rollup, `gh run view` returned GitHub API HTTP 503 for all three run-detail requests. No run was classified from that failed query and no repository/GitHub setting changed. Correction: record this audit-only failure, push the required log update, and inspect fresh run/PR state for the resulting exact head before any merge decision.
