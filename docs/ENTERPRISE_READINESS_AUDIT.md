...
- `docs/ARCHITECTURE_REPORT_ALIGNMENT_INDEX.md` records a docs/test-only architecture report alignment index that maps the uploaded architecture report phases to current repo docs without adding runtime architecture implementation, package moves, ArchUnit tooling, source scanning, ExternalSignalPort implementation, WorkloadProfile implementation, evidence packet implementation, scenario generator implementation, routing/scoring/strategy/proxy behavior changes, or claiming production readiness.
- `docs/PHASE_0_ARCHITECTURE_ADR_INDEX.md` records a docs/test-only Phase 0 architecture ADR index that names the initial planning-only ADR set recommended by the architecture report without adding accepted ADR implementation, package moves, runtime LASE enforcement, package-boundary enforcement, ExternalSignalPort implementation, WorkloadProfile implementation, evidence packet implementation, scenario generator implementation, routing/scoring/strategy/proxy behavior changes, or claiming production readiness.
- `docs/adr/ADR-0001_LAYERED_ARCHITECTURE_BOUNDARY.md` records a proposed/planning-only ADR-0001 draft for the future layered architecture boundary without adding ADR approval, package moves, ArchUnit tooling, package-boundary enforcement, runtime architecture implementation, ExternalSignalPort implementation, WorkloadProfile implementation, evidence packet implementation, scenario generator implementation, routing/scoring/strategy/proxy behavior changes, or claiming production readiness.
- `docs/adr/ADR-0002_LASE_INTEGRATION_MODEL.md` records a proposed/planning-only ADR-0002 draft for the future LASE integration model without adding ADR approval, runtime LASE enforcement, `LaseObservationPort`, replay execution, evidence packet implementation, ExternalSignalPort implementation, WorkloadProfile implementation, package-boundary enforcement, routing/scoring/strategy/proxy/API behavior changes, or claiming production readiness.
- `docs/adr/ADR-0003_EVIDENCE_AS_FIRST_CLASS_ARTIFACT.md` records a proposed/planning-only ADR-0003 draft for future evidence as a first-class architecture artifact without adding ADR approval, EvidencePacket implementation, EvidenceAssembler implementation, report generation, JSON output, storage/persistence/telemetry/audit log implementation, replay execution, routing/scoring/strategy/proxy/API behavior changes, or claiming production readiness.
- `docs/adr/ADR-0004_WORKLOAD_REALISM_AND_SCENARIO_MODELING.md` records a proposed/planning-only ADR-0004 draft for future workload realism and scenario modeling without adding ADR approval, WorkloadProfile implementation, ScenarioGenerator implementation, workload generator implementation, trace import, replay execution, EvidencePacket/report generation, JSON output, storage/persistence/telemetry implementation, routing/scoring/strategy/proxy/API behavior changes, live-cloud validation, real-tenant validation, or claiming production readiness.
- `docs/adr/ADR-0005_SAFETY_BOUNDARIES_AND_GUARDRAILS.md` records a proposed/planning-only ADR-0005 draft for future safety boundaries and guardrails without adding ADR approval, runtime enforcement, active traffic shifting, replay execution, evidence/report generation, storage/persistence, workload generation, trace import, external signal ingestion, routing/scoring/strategy/proxy/API behavior changes, live-cloud validation, real-tenant validation, production certification, or claiming production readiness.
...
- `docs/LASE_BOUNDARY_ENFORCEMENT_INVENTORY.md` records a docs/test-only current-tree inventory for future LASE boundary enforcement planning without moving classes, adding runtime interfaces, adding ArchUnit rules, enforcing packages, changing routing/scoring/strategy/proxy behavior, or claiming production readiness.
- `docs/LASE_PACKAGE_BOUNDARY_ENFORCEMENT_PLAN.md` records a docs/test-only staged plan for future package-boundary enforcement without adding ArchUnit or package-boundary tooling, moving classes, changing Maven build files, adding runtime interfaces, enforcing packages, changing routing/scoring/strategy/proxy behavior, or claiming production readiness.
- `docs/LASE_BOUNDARY_NAMING_GUARD_PLAN.md` records a docs/test-only naming guard plan for future LASE boundary vocabulary without adding runtime naming enforcement, source scanning, ArchUnit or package-boundary tooling, moving classes, changing Maven build files, adding runtime interfaces, enforcing packages, changing routing/scoring/strategy/proxy behavior, or claiming production readiness.
- `docs/LASE_NAMING_GUARD_INVENTORY.md` records a docs/test-only naming inventory for current LASE boundary naming preparation without renaming classes, moving packages, adding source-name guard tests, adding runtime naming enforcement, adding ArchUnit or package-boundary tooling, changing Maven build files, changing routing/scoring/strategy/proxy behavior, or claiming production readiness.
- `docs/LASE_SOURCE_NAME_GUARD_FEASIBILITY_PLAN.md` records a docs/test-only feasibility plan for a future narrow source-name guard without adding source scanning, runtime naming enforcement, class renames, package moves, ArchUnit or package-boundary tooling, Maven build changes, routing/scoring/strategy/proxy behavior changes, or claiming production readiness.
- `docs/SOURCE_NAME_GUARD_REVIEW_CHECKLIST.md` records a docs/test-only reviewer checklist for evaluating future source-name guard proposals without adding source scanning, source-name guard enforcement, runtime naming enforcement, class renames, package moves, ArchUnit or package-boundary tooling, Maven build changes, routing/scoring/strategy/proxy behavior changes, or claiming production readiness.
- `docs/SOURCE_NAME_GUARD_DRY_RUN_DESIGN_PLAN.md` records a docs/test-only dry-run design plan for future report-only source-name guard review without adding source scanning, dry-run commands, report generation, CI workflow changes, PR comment/report artifact behavior, source-name guard enforcement, runtime naming enforcement, ArchUnit or package-boundary tooling, Maven build changes, routing/scoring/strategy/proxy behavior changes, or claiming production readiness.
- `docs/SOURCE_NAME_GUARD_REPORT_SCHEMA_PLAN.md` records a docs/test-only report schema plan for future source-name guard dry-run reports without adding source scanning, dry-run report generation, JSON output, CI workflow changes, PR comment/report artifact behavior, source-name guard enforcement, runtime naming enforcement, ArchUnit or package-boundary tooling, Maven build changes, routing/scoring/strategy/proxy behavior changes, or claiming production readiness.
- `docs/SOURCE_NAME_GUARD_REPORT_REVIEW_CHECKLIST.md` records a docs/test-only report review checklist for future source-name guard dry-run reports without adding source scanning, dry-run report generation, JSON output, CI workflow changes, PR comment/report artifact behavior, source-name guard enforcement, runtime naming enforcement, ArchUnit or package-boundary tooling, Maven build changes, routing/scoring/strategy/proxy behavior changes, or claiming production readiness.
- `docs/SOURCE_NAME_GUARD_REPORT_SAMPLE_PLAN.md` records a docs/test-only report sample plan with static examples for future source-name guard dry-run reports without adding source scanning, dry-run report generation, JSON output files, CI workflow changes, PR comment/report artifact behavior, source-name guard enforcement, runtime naming enforcement, ArchUnit or package-boundary tooling, Maven build changes, routing/scoring/strategy/proxy behavior changes, or claiming production readiness.
- `docs/SOURCE_NAME_GUARD_REPORT_ACCEPTANCE_CRITERIA_PLAN.md` records a docs/test-only report acceptance criteria plan for future source-name guard dry-run reports without adding source scanning, dry-run report generation, JSON output files, CI workflow changes, PR comment/report artifact behavior, source-name guard enforcement, runtime naming enforcement, ArchUnit or package-boundary tooling, Maven build changes, routing/scoring/strategy/proxy behavior changes, or claiming production readiness.
- `docs/SOURCE_NAME_GUARD_RULE_CATALOG_PLAN.md` records a docs/test-only rule catalog plan for candidate future source-name guard rules without adding source scanning, dry-run report generation, JSON output files, CI workflow changes, PR comment/report artifact behavior, source-name guard enforcement, runtime naming enforcement, ArchUnit or package-boundary tooling, Maven build changes, routing/scoring/strategy/proxy behavior changes, or claiming production readiness.
- `docs/SOURCE_NAME_GUARD_RULE_REVIEW_CHECKLIST.md` records a docs/test-only rule review checklist for evaluating individual future source-name guard rule candidates without adding source scanning, dry-run report generation, JSON output files, CI workflow changes, PR comment/report artifact behavior, source-name guard enforcement, runtime naming enforcement, ArchUnit or package-boundary tooling, Maven build changes, routing/scoring/strategy/proxy behavior changes, or claiming production readiness.
- `docs/SOURCE_NAME_GUARD_ALLOWLIST_DESIGN_PLAN.md` records a docs/test-only allowlist design plan for future source-name guard allowlist semantics without adding allowlist files, source scanning, dry-run report generation, JSON/YAML/TOML output files, CI workflow changes, PR comment/report artifact behavior, source-name guard enforcement, runtime naming enforcement, ArchUnit or package-boundary tooling, Maven build changes, routing/scoring/strategy/proxy behavior changes, or claiming production readiness.
...
- `/adaptive-routing-scenarios.html`, `GET /api/enterprise-lab/adaptive-routing-scenario-summary`, `GET /api/enterprise-lab/adaptive-routing-scenario-detail`, `GET /api/enterprise-lab/adaptive-routing-scenario-evidence-packet`, `GET /api/enterprise-lab/adaptive-routing-scenario-gate-evaluation`, and `GET /api/enterprise-lab/adaptive-routing-strategy-comparison-matrix` expose deterministic local synthetic selected-server distribution evidence, strategy explanation drilldowns, an in-memory reviewer packet shape, local reviewer gate findings, and a strategy-by-scenario comparison matrix from existing routing strategies without production benchmark, live traffic, or active CI enforcement claims.
