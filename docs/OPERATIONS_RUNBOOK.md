...
Private-network live traffic execution is not implemented yet. The default-off live gate properties `loadbalancerpro.proxy.private-network-live-validation.enabled=false` and `loadbalancerpro.proxy.private-network-live-validation.operator-approved=false` are implemented for offline review only. `PrivateNetworkLiveValidationGate` requires those flags to be true, config validation to be enabled, proxy mode to be enabled, and every backend literal to pass the classifier before it returns an allowed decision. It does not send traffic, resolve DNS, probe reachability, discover hosts, scan ports, or start Postman/smoke execution. Before a future traffic executor can be added, the live gate still must prove bounded timeout behavior, redacted ignored evidence, API-key/OAuth2 boundary proof, and fail-closed reload/startup behavior.
