...
- `/adaptive-routing-scenarios.html`, `GET /api/enterprise-lab/adaptive-routing-scenario-summary`, and `GET /api/enterprise-lab/adaptive-routing-scenario-detail` expose deterministic local synthetic selected-server distribution evidence and strategy explanation drilldowns from existing routing strategies without production benchmark or live traffic claims.
