...
- `/adaptive-routing-scenarios.html` and `GET /api/enterprise-lab/adaptive-routing-scenario-summary` expose deterministic local synthetic selected-server distribution evidence from existing routing strategies without production benchmark or live traffic claims.
