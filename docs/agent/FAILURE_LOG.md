...
- P-4.1: merge-main CI failed only the inherited `<5%` access-log overhead gate after the identical merge tree passed two Linux PR runs and repeated local forks. The three exact-tree Linux medians were `2.530%`, `4.721%`, and `6.939%`; each timed interval was only about 120 ms, so the repair triples the measured saturation window without changing the nine-sample median, zero-drop assertion, workload shape, or threshold.
- P-4.2 prerequisite: the inherited `<5%` access-log microbenchmark continued to produce runner-sensitive pass/fail evidence after longer intervals, blocking a trustworthy source-main checkpoint. Isolated PR `#553` moved strict timing out of default Surefire into an explicit five-fork diagnostic lane with exact artifacts while preserving deterministic zero-drop, ordering, capacity, and shutdown contracts in the default suite; it made no production-performance claim and changed no runtime behavior.
