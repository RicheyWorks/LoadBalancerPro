...
Before any future private-network live traffic execution is implemented, [`PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md`](PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md) must be satisfied. The implemented offline gate uses default-off `loadbalancerpro.proxy.private-network-live-validation.enabled=false` and `loadbalancerpro.proxy.private-network-live-validation.operator-approved=false` properties, requires explicit operator approval plus classifier approval for every literal backend URL, and still requires future bounded timeout behavior, redacted ignored `target/` evidence, API-key/OAuth2 boundary proof, and continued no-DNS/no-discovery/no-scanning rules before traffic can be added.
