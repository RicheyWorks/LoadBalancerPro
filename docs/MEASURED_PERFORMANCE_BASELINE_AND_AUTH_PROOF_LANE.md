...
- [`CI_EVIDENCE_GATE_READINESS_LANE.md`](CI_EVIDENCE_GATE_READINESS_LANE.md), `/ci-evidence-gate.html`, and `GET /api/enterprise-lab/ci-evidence-gate-summary` explain and prototype how these local evidence outputs could become inputs to a future CI evidence gate without enforcing it today.
