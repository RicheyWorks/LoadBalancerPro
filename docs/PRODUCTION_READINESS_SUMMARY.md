...
| Adaptive-routing lab evidence | `POST /api/allocate/evaluate` exposes optional `laseShadow` and `lasePolicy` summaries. The policy gate supports `off`, `shadow`, `recommend`, and `active-experiment`, defaults safe/off, keeps `shadow` and `recommend` non-mutating, and allows active-experiment changes only when explicit config and health/eligibility/capacity/freshness/conflict/rollback/bounded-context gates pass. The Enterprise Lab workflow adds `GET /api/lab/scenarios`, `POST /api/lab/runs`, `GET /api/lab/policy`, `GET /api/lab/audit-events`, `GET /api/lab/metrics`, `GET /api/lab/metrics/prometheus`, bounded in-memory run/audit/metrics retrieval, scorecards, `/enterprise-lab.html`, ignored `target/enterprise-lab-runs/` evidence export, controlled policy evidence under `target/controlled-adaptive-routing/`, observability evidence under `target/enterprise-lab-observability/`, dashboard JSON, alert examples, and SLO templates. Default runtime allocation behavior remains unchanged, and lab metrics are process-local rather than production SLO certification. | [`API_CONTRACTS.md`](API_CONTRACTS.md), [`CONTROLLED_ACTIVE_LASE_POLICY_GATE.md`](CONTROLLED_ACTIVE_LASE_POLICY_GATE.md), [`DEMO_WALKTHROUGH.md`](DEMO_WALKTHROUGH.md), [`SRE_DEMO_HIGHLIGHTS.md`](SRE_DEMO_HIGHLIGHTS.md), [`observability/grafana-enterprise-lab-dashboard.json`](observability/grafana-enterprise-lab-dashboard.json), [`observability/enterprise-lab-alerts.yml`](observability/enterprise-lab-alerts.yml), [`observability/SLO_TEMPLATES.md`](observability/SLO_TEMPLATES.md), `LaseAllocationShadowIntegrationTest`, `AdaptiveRoutingPolicyEngineTest`, `EnterpriseLabControllerTest`, `AdaptiveRoutingObservabilityMetricsTest` |
| Performance baseline lane | Deterministic fixture catalog, source-visible local/package runner, latency/error-rate summaries, warning-only thresholds, dashboard-ready JSON, and ignored `target/performance-baseline/` evidence now exist. The lane measures local loopback behavior only and does not claim production SLOs, capacity, cloud performance, or real user traffic proof. | [`../evidence/PERFORMANCE_BASELINE.md`](../evidence/PERFORMANCE_BASELINE.md), [`OBSERVABILITY.md`](OBSERVABILITY.md), `docs/performance/performance-fixtures.json`, `docs/performance/performance-thresholds.example.json`, `PerformanceBaselineFixtureCatalogTest` |
| Enterprise auth proof lane | Mock IdP/JWKS claim fixtures, role lifecycle guidance, token lifetime/key-rotation guidance, source-visible proof script, and ignored `target/enterprise-auth-proof/` evidence now prove dedicated role claims, scope-only denial, missing/ambiguous claim denial, expired token rejection, wrong issuer/audience rejection, and prod-style API-key boundary behavior without real tenant secrets. | [`ENTERPRISE_AUTH_PROOF_LANE.md`](ENTERPRISE_AUTH_PROOF_LANE.md), [`API_SECURITY.md`](API_SECURITY.md), [`IDP_CLAIM_MAPPING_EXAMPLES.md`](IDP_CLAIM_MAPPING_EXAMPLES.md), `EnterpriseAuthProofLaneTest`, `OAuth2AuthorizationTest` |
...
- Controlled active LASE policy evidence: run `scripts/smoke/controlled-adaptive-routing-policy.ps1 -Package` and inspect ignored output under `target/controlled-adaptive-routing/`; it covers `off`, `shadow`, `recommend`, `active-experiment`, guardrail-blocked scenarios, rollback reasons, and audit event summaries.
- Enterprise Lab observability evidence: run `scripts/smoke/enterprise-lab-observability-pack.ps1 -Package` and inspect ignored output under `target/enterprise-lab-observability/`; it captures protected lab metrics JSON, Prometheus-style sample text, dashboard/alert/SLO references, and a manifest without requiring live Prometheus, Grafana, cloud, or production SLO certification.
- Performance baseline evidence: run `scripts/smoke/performance-baseline.ps1 -Package` and inspect ignored output under `target/performance-baseline/`; it captures local loopback latency/error-rate summaries, warning-only threshold results, dashboard-ready JSON, and a manifest without requiring live cloud, private network, or production SLO claims.
- Enterprise auth proof evidence: run `scripts/smoke/enterprise-auth-proof.ps1 -Package` and inspect ignored output under `target/enterprise-auth-proof/`; it captures prod-style API-key boundary checks and mocked OAuth2 role-claim proof without real tenant secrets or live IdP calls.
