...
| Are proxy health and metrics visible? | Status endpoint and operator page | [`REVERSE_PROXY_HEALTH_AND_METRICS.md`](REVERSE_PROXY_HEALTH_AND_METRICS.md), [`PROXY_OPERATOR_STATUS_UI.md`](PROXY_OPERATOR_STATUS_UI.md) | `/api/proxy/status`, `/proxy-status.html` | Health, counters, selected upstream, route/backend summary, boundary mode, and status JSON can be reviewed read-only | Durable monitoring, alerting, or reset/admin controls |
