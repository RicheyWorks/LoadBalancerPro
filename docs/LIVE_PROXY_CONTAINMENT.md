...
`POST /api/proxy/private-network-live-validation` is the current operator-trigger command contract only. It is protected by the existing prod API-key/OAuth2 boundary, validates request shape and offline gate state, returns `trafficExecuted=false` and `evidenceWritten=false`, and always reports that traffic execution is not wired in this release. It does not call `PrivateNetworkLiveValidationExecutor`, write evidence, or send private-LAN/public traffic.
