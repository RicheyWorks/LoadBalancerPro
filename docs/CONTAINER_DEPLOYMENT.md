...
Run the app locally with proxy mode disabled by default:
...
Use placeholder local secrets for validation. Keep real secrets in your deployment secret system, not in Git, Dockerfile layers, image labels, or shell history.
...
```bash
curl -i http://127.0.0.1:8080/proxy/api/smoke
