...
The read-only `/api/routing/compare` response exposes the useful controlled-lab evidence surfaces: `results[].decisionVector`, `results[].dominantFactorAnalysis`, `results[].decisionDeltaAnalysis`, `results[].decisionReplaySnapshot`, `results[].decisionReplayReconstructionTrace`, `results[].decisionReplayCapsule`, `results[].decisionReplayReadinessChecklist`, and `results[].decisionReplayEvidenceSourceMap`.

These surfaces explain visible local inputs and current calculator contributions. They do not expose hidden production scoring, execute replay, perform what-if mutation, persist evidence, change routing behavior, or prove production readiness. The former boundary/inventory/lane/reviewer metadata-about-metadata chain was retired because it restated existing objects without adding decision evidence.

The retained contracts are documented in [`ENTERPRISE_LAB_DECISION_VECTOR.md`](ENTERPRISE_LAB_DECISION_VECTOR.md), [`ENTERPRISE_LAB_DOMINANT_FACTOR_ANALYSIS.md`](ENTERPRISE_LAB_DOMINANT_FACTOR_ANALYSIS.md), [`ENTERPRISE_LAB_DECISION_DELTA_ANALYSIS.md`](ENTERPRISE_LAB_DECISION_DELTA_ANALYSIS.md), [`ENTERPRISE_LAB_DECISION_REPLAY_SNAPSHOT.md`](ENTERPRISE_LAB_DECISION_REPLAY_SNAPSHOT.md), [`ENTERPRISE_LAB_DECISION_REPLAY_RECONSTRUCTION_TRACE.md`](ENTERPRISE_LAB_DECISION_REPLAY_RECONSTRUCTION_TRACE.md), [`ENTERPRISE_LAB_DECISION_REPLAY_CAPSULE.md`](ENTERPRISE_LAB_DECISION_REPLAY_CAPSULE.md), [`ENTERPRISE_LAB_DECISION_REPLAY_READINESS_CHECKLIST.md`](ENTERPRISE_LAB_DECISION_REPLAY_READINESS_CHECKLIST.md), and [`ENTERPRISE_LAB_DECISION_REPLAY_EVIDENCE_SOURCE_MAP.md`](ENTERPRISE_LAB_DECISION_REPLAY_EVIDENCE_SOURCE_MAP.md).

The Decision Vector remains the structured explanation contract for one controlled lab routing decision. It keeps the selected candidate vector and non-selected candidate vectors comparable while unknown or unexposed signals remain explicit.

`ServerScoreCalculator` factor contribution extraction is additive. Candidate factor contribution summaries can attach those calculator explanations to selected and non-selected candidate vectors without retuning weights or changing the selected backend.

The snapshot keeps a deterministic local fingerprint from already-built compare evidence only; the trace keeps deterministic reconstruction evidence steps; the capsule provides canonical evidence packaging; and the checklist reports lab replay-readiness checklist status. The source map maps already-built evidence source fields to replay/readiness artifacts without new fingerprint generation, without replay execution, without what-if mutation, without scoring changes, and without routing behavior changes.

The retained surfaces are additive, preserving existing fields, routing behavior, scoring behavior, and strategy weights. Replay execution and what-if execution remain future/not implemented.
