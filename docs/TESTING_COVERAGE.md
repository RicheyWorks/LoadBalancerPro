...
- Default tests exercise the real loopback proxy path and verify that the default executable contains its status/config/runtime classes while excluding simulation, lab, GUI, demo CLI, synthetic monitor, and lab-only dependency entries. The separate `-P lab` build retains those controlled tools. This remains local regression evidence, not a production gateway capacity or SLO claim.
