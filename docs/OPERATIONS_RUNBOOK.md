...
Private-network live validation is not implemented yet. Before it can be added, the live gate must require explicit operator approval, default-off live flags, classifier approval for every backend literal, bounded timeout behavior, redacted ignored evidence, API-key/OAuth2 boundary proof, no DNS, no discovery, no scanning, and fail-closed reload/startup behavior.
