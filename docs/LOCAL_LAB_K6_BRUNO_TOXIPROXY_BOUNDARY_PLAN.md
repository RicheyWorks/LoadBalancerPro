...
This document is a docs/test-only k6/Bruno/Toxiproxy implementation boundary plan for local-lab tooling. It now distinguishes the first separately scoped optional k6 smoke script skeleton, first separately scoped optional Bruno collection skeleton, first separately scoped optional Toxiproxy config skeleton, [`LOCAL_LAB_DOCKER_COMPOSE_BOUNDARY_DESIGN.md`](LOCAL_LAB_DOCKER_COMPOSE_BOUNDARY_DESIGN.md), the first separately scoped optional local-lab Compose skeleton in [`LOCAL_LAB_DOCKER_COMPOSE_SKELETON.md`](LOCAL_LAB_DOCKER_COMPOSE_SKELETON.md), the future app-service preflight checklist in [`LOCAL_LAB_DOCKER_COMPOSE_APP_SERVICE_PREFLIGHT_CHECKLIST.md`](LOCAL_LAB_DOCKER_COMPOSE_APP_SERVICE_PREFLIGHT_CHECKLIST.md), and the future-change readiness gate in [`LOCAL_LAB_DOCKER_COMPOSE_READINESS_GATE.md`](LOCAL_LAB_DOCKER_COMPOSE_READINESS_GATE.md) from still-future expanded k6, expanded Bruno, expanded Toxiproxy fault execution, app container orchestration, production Docker packaging, and automated Docker/Docker Compose work.

PR #270 added a docs/test-only k6/Bruno/Toxiproxy implementation boundary plan and scenario design specs before tool files existed. PR #272 added one optional local-lab k6 smoke script skeleton at [`../lab/k6/local-lab-smoke.js`](../lab/k6/local-lab-smoke.js). PR #273 added one optional local-lab Bruno collection skeleton at [`../lab/bruno/local-lab-smoke/`](../lab/bruno/local-lab-smoke/). PR #274 added one optional local-lab Toxiproxy config skeleton at [`../lab/toxiproxy/local-lab-toxiproxy.json`](../lab/toxiproxy/local-lab-toxiproxy.json). [`LOCAL_LAB_MANUAL_TOOLING_INDEX.md`](LOCAL_LAB_MANUAL_TOOLING_INDEX.md) is the unified docs/test-only reviewer index for those current manual tool skeletons, and [`LOCAL_LAB_MANUAL_TOOLING_RUNBOOK.md`](LOCAL_LAB_MANUAL_TOOLING_RUNBOOK.md) is the companion manual reviewer/operator runbook for inspection-only review and optional local-only manual use. [`LOCAL_LAB_DOCKER_COMPOSE_SKELETON.md`](LOCAL_LAB_DOCKER_COMPOSE_SKELETON.md) documents one optional local-lab Compose skeleton at [`../lab/docker-compose/local-lab-compose.yml`](../lab/docker-compose/local-lab-compose.yml), [`LOCAL_LAB_DOCKER_COMPOSE_MANUAL_RUNBOOK.md`](LOCAL_LAB_DOCKER_COMPOSE_MANUAL_RUNBOOK.md) gives the docs/test-only Compose-specific inspection checklist, [`LOCAL_LAB_DOCKER_COMPOSE_APP_SERVICE_PREFLIGHT_CHECKLIST.md`](LOCAL_LAB_DOCKER_COMPOSE_APP_SERVICE_PREFLIGHT_CHECKLIST.md) defines exact preflight proof before any future app-service PR, and [`LOCAL_LAB_DOCKER_COMPOSE_READINESS_GATE.md`](LOCAL_LAB_DOCKER_COMPOSE_READINESS_GATE.md) defines the future-change checklist before editing Compose or adding services. See [`LOCAL_LAB_K6_SMOKE_SCRIPT.md`](LOCAL_LAB_K6_SMOKE_SCRIPT.md), [`LOCAL_LAB_BRUNO_COLLECTION.md`](LOCAL_LAB_BRUNO_COLLECTION.md), [`LOCAL_LAB_TOXIPROXY_CONFIG.md`](LOCAL_LAB_TOXIPROXY_CONFIG.md), [`LOCAL_LAB_K6_SCENARIO_DESIGN.md`](LOCAL_LAB_K6_SCENARIO_DESIGN.md), [`LOCAL_LAB_BRUNO_COLLECTION_DESIGN.md`](LOCAL_LAB_BRUNO_COLLECTION_DESIGN.md), and [`LOCAL_LAB_TOXIPROXY_FAULT_MODEL_DESIGN.md`](LOCAL_LAB_TOXIPROXY_FAULT_MODEL_DESIGN.md). The skeletons are not CI-gated, not wired into Maven, and must target local/lab-owned loopback endpoints by default. Bruno is not Toxiproxy integration and not k6 execution. Toxiproxy is manual-only, not wired into the application, not wired into Maven, not wired into k6 execution, not wired into Bruno execution, does not start Toxiproxy by itself, and does not start the application. The optional Compose skeleton is manual-only, local-lab-only, loopback-bound, not production Docker packaging, not an app container, not k6 execution, not Bruno execution, not automated execution, and not production runtime behavior. This follow-up adds no automatically run scripts, no CI jobs, no Maven dependencies, no production endpoints, no production listeners, no production runtime behavior, no replay execution, no evidence/report generation, no storage/export behavior, no load/stress/benchmark testing, no throughput evidence, and no p95/p99 evidence.
...
Expanded Toxiproxy fault execution remains future-only tooling in this plan. Docker/Docker Compose is future-only unless separately scoped. [`LOCAL_LAB_DOCKER_COMPOSE_BOUNDARY_DESIGN.md`](LOCAL_LAB_DOCKER_COMPOSE_BOUNDARY_DESIGN.md) is design-only and future-only for broader orchestration; [`LOCAL_LAB_DOCKER_COMPOSE_SKELETON.md`](LOCAL_LAB_DOCKER_COMPOSE_SKELETON.md) is the separately scoped first optional local-lab Compose skeleton; [`LOCAL_LAB_DOCKER_COMPOSE_APP_SERVICE_BOUNDARY_DESIGN.md`](LOCAL_LAB_DOCKER_COMPOSE_APP_SERVICE_BOUNDARY_DESIGN.md) is future-only app-service boundary design; [`LOCAL_LAB_DOCKER_COMPOSE_APP_SERVICE_PREFLIGHT_CHECKLIST.md`](LOCAL_LAB_DOCKER_COMPOSE_APP_SERVICE_PREFLIGHT_CHECKLIST.md) is the exact app-service preflight checklist; [`LOCAL_LAB_DOCKER_COMPOSE_READINESS_GATE.md`](LOCAL_LAB_DOCKER_COMPOSE_READINESS_GATE.md) is the readiness gate for any future Compose file, app service, k6 runner, Bruno automation, or Toxiproxy expansion change. App container orchestration, k6 runner services, Bruno runner services, production Docker packaging, CI execution, Maven execution, and production runtime behavior still require a separate implementation PR. k6 is limited to the one optional local-lab smoke script skeleton until a later sprint separately scopes expanded k6 scenario files. Bruno is limited to the one optional local-lab Bruno collection skeleton until a later sprint separately scopes expanded Bruno collections. Toxiproxy is limited to the one optional manual loopback-only config skeleton and one optional manual loopback-only Compose mount until a later sprint separately scopes fault execution. Passing this documentation guard only means the implementation boundary is reviewer-readable; it is not production proof.
...
- Docker Compose currently has one optional local-lab skeleton only, documented in [`LOCAL_LAB_DOCKER_COMPOSE_SKELETON.md`](LOCAL_LAB_DOCKER_COMPOSE_SKELETON.md); broader app orchestration, k6 runner services, Bruno runner services, CI-gating, Maven wiring, and production runtime behavior remain separately scoped.
...
The current local lab has one optional k6 smoke script skeleton, one optional local-lab Bruno collection skeleton, one optional local-lab Toxiproxy config skeleton, and one optional local-lab Docker Compose skeleton only. Bruno is manual only, not CI-gated, not Dockerized, not Toxiproxy integration, not k6 execution, and must use local/lab-owned loopback endpoints by default. Toxiproxy is manual-only, not CI-gated, not wired into the application, not wired into Maven, not wired into k6 execution, not wired into Bruno execution, does not start the application, and must use local/lab-owned loopback endpoints by default. The Compose skeleton is optional, manual-only, local-lab-only, loopback-bound, not CI-gated, not wired into Maven, not production Docker packaging, not production runtime behavior, not an app service, not a k6 runner, and not a Bruno runner. The current local lab still does not use automatically run scripts, CI jobs, production infrastructure, production traffic, replay execution, evidence/report generation, storage, or export behavior.
...
- one optional local-lab Compose skeleton only at [`../lab/docker-compose/local-lab-compose.yml`](../lab/docker-compose/local-lab-compose.yml);
- no app container orchestration yet;
- no production Docker packaging;
- no k6 runner service;
- no Bruno runner service;
- no CI or Maven execution;
...
- future Compose file and service-change gates live in [`LOCAL_LAB_DOCKER_COMPOSE_READINESS_GATE.md`](LOCAL_LAB_DOCKER_COMPOSE_READINESS_GATE.md);
...
- the optional local-lab Compose skeleton publishes non-loopback ports, contains `0.0.0.0`, includes production-looking domains, introduces secrets, adds an app service, adds a k6 runner service, adds a Bruno runner service, becomes CI-gated, becomes wired into Maven, changes production Docker packaging, or changes production runtime behavior;
...
- Is this limited to the one optional k6 smoke skeleton, one optional Bruno collection skeleton, one optional Toxiproxy config skeleton, one optional local-lab Compose skeleton, plus docs/tests?
...
- Is the optional local-lab Compose skeleton still manual-only, loopback-bound, not CI-gated, not wired into Maven, and not production runtime behavior?
