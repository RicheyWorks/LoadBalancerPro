...
- [`LOCAL_LAB_DOCKER_COMPOSE_APP_SERVICE_BOUNDARY_DESIGN.md`](LOCAL_LAB_DOCKER_COMPOSE_APP_SERVICE_BOUNDARY_DESIGN.md) is future-only app-service boundary design. It does not add an app service, change this config, add Docker packaging, wire Compose into CI or Maven, or change production runtime behavior.
- [`LOCAL_LAB_DOCKER_COMPOSE_APP_SERVICE_PREFLIGHT_CHECKLIST.md`](LOCAL_LAB_DOCKER_COMPOSE_APP_SERVICE_PREFLIGHT_CHECKLIST.md) defines the exact preflight proof before any future app-service PR. It does not change this config, add an app service, add Toxiproxy execution, or broaden targets beyond loopback/local use.
- [`LOCAL_LAB_DOCKER_COMPOSE_READINESS_GATE.md`](LOCAL_LAB_DOCKER_COMPOSE_READINESS_GATE.md) defines what a future PR must prove before changing the Compose file or expanding Toxiproxy services/toxics. It does not change this config, add Toxiproxy execution, add CI/Maven wiring, or broaden targets beyond loopback/local use.
