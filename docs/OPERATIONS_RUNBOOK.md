...
For OAuth2 IdP role-claim examples, scope-only denial expectations, and local/mock validation without real IdP secrets, see [`IDP_CLAIM_MAPPING_EXAMPLES.md`](IDP_CLAIM_MAPPING_EXAMPLES.md).

For CodeQL, Dependency Review, Trivy, SBOM, and dependency finding ownership, accepted-risk rationale, false-positive rationale, remediation targets, and evidence locations, see [`DEPENDENCY_SAST_RISK_WORKFLOW.md`](DEPENDENCY_SAST_RISK_WORKFLOW.md).
...
For the Enterprise Lab workflow, open `http://localhost:8080/enterprise-lab.html` in local/demo mode, call `GET /api/lab/scenarios` to list deterministic adaptive-routing scenarios, call `POST /api/lab/runs` to compare baseline, shadow, recommend, and active-experiment outcomes with scorecards, and call `GET /api/lab/policy` plus `GET /api/lab/audit-events` to inspect the controlled policy status and bounded audit trail. Call `GET /api/lab/metrics` or `GET /api/lab/metrics/prometheus` to inspect process-local lab-grade counters for lab runs, policy decisions, guardrails, rollback/fail-closed events, audit retention/drops, and rate-limit interactions. Runtime run storage, audit events, and lab metrics are process-local, in-memory, and bounded by retained-run, audit-event, and scenario-count limits. For ignored reviewer evidence, run `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\smoke\enterprise-lab-workflow.ps1 -Package`; it writes scenario catalog JSON, run JSON, Markdown scorecard summary, and metadata only under `target/enterprise-lab-runs/`. For controlled policy evidence, run `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\smoke\controlled-adaptive-routing-policy.ps1 -Package`; it writes JSON and Markdown policy evidence only under `target/controlled-adaptive-routing/`. For observability evidence, run `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\smoke\enterprise-lab-observability-pack.ps1 -Package`; it writes metrics JSON, Prometheus-style sample text, Markdown summary, and a manifest under `target/enterprise-lab-observability/`. For local performance evidence, run `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\smoke\performance-baseline.ps1 -Package`; it writes latency, error-rate, threshold, and dashboard-ready JSON/Markdown under `target/performance-baseline/`. For mocked enterprise auth proof, run `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\smoke\enterprise-auth-proof.ps1 -Package`; it writes API-key boundary and mocked OAuth2 role-claim proof under `target/enterprise-auth-proof/`. These workflows do not call `CloudManager`, mutate cloud state, use external network services or real IdP tenants, create releases/tags/assets, or publish containers.

Container/default deployment mode is protected by the prod API-key profile because the checked-in Dockerfile defaults `SPRING_PROFILES_ACTIVE=prod`. Set `LOADBALANCERPRO_API_KEY` through the runtime environment or deployment secret system before expecting `/api/**`, proxy, OpenAPI, or Swagger requests to succeed. The explicit public prod/cloud-sandbox API-key exceptions are `GET /api/health` and unauthenticated `OPTIONS` preflight requests; Actuator health/info exposure is configured separately by profile. Local developer mode remains intentionally permissive for source-checkout demos; do not expose local/demo mode on public interfaces.
...
For reviewer/operator dry-run evidence without sending traffic, run `mvn -Dtest=PrivateNetworkProxyDryRunEvidenceTest test`, then inspect `target/proxy-evidence/private-network-validation-dry-run.md` and `target/proxy-evidence/private-network-validation-dry-run.json`. The recipe classifies allowed loopback/private literals and rejected public/domain/user-info/unsupported/malformed samples, keeps evidence under ignored `target/` output, and writes no API keys or secrets.

For report-only live gate visibility, inspect `/api/proxy/status.privateNetworkLiveValidation` in the same status response. In prod/cloud-sandbox API-key mode, include `X-API-Key` as you would for any proxy status request. The report lists the live flag, operator approval flag, config-validation flag, proxy enabled state, classifier decisions, and reason codes, but it always reports `trafficExecuted=false` and `trafficExecution="traffic not executed by this report"` because it does not call the executor or send validation traffic.

For the future operator-triggered live validation command contract, use `POST /api/proxy/private-network-live-validation` only to validate request shape and the offline gate. The request accepts a simple `requestPath`, optional `evidenceRequested`, and optional operator acknowledgement. It returns `INVALID_REQUEST`, `BLOCKED_BY_GATE`, or `NOT_IMPLEMENTED` with `accepted=false`, `executable=false`, `trafficExecuted=false`, and `evidenceWritten=false`; traffic execution is not wired in this release. The response names the planned future evidence files under ignored `target/proxy-evidence/` (`private-network-live-validation.md`, `private-network-live-validation.json`, and `private-network-live-validation-audit.jsonl`) and requires redaction, but the runtime command writes none of them. In prod/cloud-sandbox API-key mode, include `X-API-Key`; in OAuth2 mode, use the configured allocation role. This command does not call the executor, send private-LAN/public traffic, write evidence, write audit files, or echo secrets.

Runtime private-LAN live traffic execution is not implemented yet. The default-off live gate properties `loadbalancerpro.proxy.private-network-live-validation.enabled=false` and `loadbalancerpro.proxy.private-network-live-validation.operator-approved=false` are implemented for review and test-only proof. `PrivateNetworkLiveValidationGate` requires those flags to be true, config validation to be enabled, proxy mode to be enabled, and every backend literal to pass the classifier before it returns an allowed decision. `PrivateNetworkLiveValidationExecutor` requires that allowed gate result before any bounded validation request can be delegated to its transport. For the loopback-only proof, run `mvn -Dtest=PrivateNetworkLiveValidationExecutorTest test`, then inspect `target/proxy-evidence/private-network-live-loopback-validation.md` and `target/proxy-evidence/private-network-live-loopback-validation.json`. That proof uses a JUnit-only JDK `HttpServer` on loopback, writes redacted ignored `target/` evidence, rejects unsafe validation paths before transport, allowlists validation request/response summary headers, reports redirects without following public `Location` targets, and is not called by app startup, Postman, smoke scripts, or proxy routing. Broader private-network live execution still requires a separate approved task with API-key/OAuth2 boundary proof and fail-closed reload/startup behavior.

For generated API documentation, explicit local `auth.mode=none` keeps `/v3/api-docs` and Swagger UI public for loopback developer review. API-key mode requires `X-API-Key` for `/v3/api-docs`, `/v3/api-docs/**`, `/swagger-ui.html`, and `/swagger-ui/**` regardless of profile. Use Postman or curl with the configured header for API-key reviews; do not make Actuator metrics or Prometheus public to compensate for gated docs.

For Enterprise Lab APIs in prod or cloud-sandbox API-key mode, include `X-API-Key` for `/api/lab/scenarios`, `/api/lab/runs`, retained-run lookups, `/api/lab/policy`, `/api/lab/audit-events`, `/api/lab/metrics`, and `/api/lab/metrics/prometheus`. In OAuth2 mode, scenario catalog reads use configured read roles and run creation/retrieval plus policy/audit/metrics status use the allocation role. Local/default mode remains intentionally convenient and should stay loopback/private.

Controlled LASE policy mode defaults are safe: `loadbalancerpro.lase.policy.mode=off`, `loadbalancerpro.lase.policy.active-experiment-enabled=false`, and `loadbalancerpro.lase.policy.max-audit-events=100`. `shadow` and `recommend` explain LASE decisions without changing the final allocation. `active-experiment` must be explicitly enabled and still degrades to baseline when health, eligibility, capacity, freshness, conflict, all-unhealthy, rollback, or bounded-context gates fail. Treat rollback reasons as operator evidence, not as production remediation automation.
...
In explicit local `auth.mode=none`, those evidence-training routes remain public for loopback onboarding convenience. In API-key mode, all `/api/evidence-training/**` routes require `X-API-Key` regardless of profile; in OAuth2 mode they require the allocation role.
...
Open `http://localhost:8080/routing-demo.html` for the Enterprise Lab routing cockpit legacy route, or run the legacy `Routing Decision Demo` folder in the same Postman collection. This path uses the existing `POST /api/routing/compare` endpoint with deterministic controlled lab server telemetry. It compares `TAIL_LATENCY_POWER_OF_TWO`, `WEIGHTED_LEAST_LOAD`, `WEIGHTED_LEAST_CONNECTIONS`, `WEIGHTED_ROUND_ROBIN`, `ROUND_ROBIN`, and `CONSISTENT_HASH`, then displays selected servers, strategy reasons, candidate lists, scores, raw JSON, copyable curl, copyable payload, proof notes, and a normalized copyable summary. Consistent hashing here uses a deterministic synthetic comparison key rather than live proxy request metadata. It is read-only lab recommendation output, not certification, not benchmark proof, and not a production-routing benchmark.
...
See [`OPERATOR_DEMO_WALKTHROUGH.md`](OPERATOR_DEMO_WALKTHROUGH.md), [`OPERATOR_GUIDED_WALKTHROUGH.md`](OPERATOR_GUIDED_WALKTHROUGH.md), [`OPERATOR_NAVIGATION_READINESS.md`](OPERATOR_NAVIGATION_READINESS.md), [`OPERATOR_PACKAGING.md`](OPERATOR_PACKAGING.md), [`LOCAL_ARTIFACT_VERIFICATION.md`](LOCAL_ARTIFACT_VERIFICATION.md), [`CI_ARTIFACT_CONSUMER_GUIDE.md`](CI_ARTIFACT_CONSUMER_GUIDE.md), [`RELEASE_CANDIDATE_DRY_RUN.md`](RELEASE_CANDIDATE_DRY_RUN.md), [`OPERATOR_INSTALL_RUN_MATRIX.md`](OPERATOR_INSTALL_RUN_MATRIX.md), [`RELEASE_INTENT_CHECKLIST.md`](RELEASE_INTENT_CHECKLIST.md), [`PACKAGE_NAMING.md`](PACKAGE_NAMING.md), [`OPERATOR_SCENARIO_GALLERY.md`](OPERATOR_SCENARIO_GALLERY.md), [`OPERATOR_COMPARISON_MATRIX.md`](OPERATOR_COMPARISON_MATRIX.md), [`OPERATOR_REPLAY_MODE.md`](OPERATOR_REPLAY_MODE.md), [`OPERATOR_REVIEW_PACKET.md`](OPERATOR_REVIEW_PACKET.md), [`OPERATOR_API_CONTRACT_TRACE.md`](OPERATOR_API_CONTRACT_TRACE.md), [`OPERATOR_EXPLANATION_DRILLDOWN.md`](OPERATOR_EXPLANATION_DRILLDOWN.md), [`POSTMAN_EVIDENCE_TRAINING.md`](POSTMAN_EVIDENCE_TRAINING.md), [`POSTMAN_ROUTING_DEMO.md`](POSTMAN_ROUTING_DEMO.md), [`POSTMAN_LOAD_BALANCING_COCKPIT.md`](POSTMAN_LOAD_BALANCING_COCKPIT.md), [`REVERSE_PROXY_MODE.md`](REVERSE_PROXY_MODE.md), [`REVERSE_PROXY_HEALTH_AND_METRICS.md`](REVERSE_PROXY_HEALTH_AND_METRICS.md), [`REVERSE_PROXY_RESILIENCE.md`](REVERSE_PROXY_RESILIENCE.md), [`PROXY_DEMO_STACK.md`](PROXY_DEMO_STACK.md), [`PROXY_DEMO_FIXTURE_LAUNCHER.md`](PROXY_DEMO_FIXTURE_LAUNCHER.md), [`PROXY_OPERATOR_STATUS_UI.md`](PROXY_OPERATOR_STATUS_UI.md), [`PROXY_STRATEGY_DEMO_LAB.md`](PROXY_STRATEGY_DEMO_LAB.md), [`REMEDIATION_REPORT_CLI.md`](REMEDIATION_REPORT_CLI.md), [`EVIDENCE_POLICY_TEMPLATES.md`](EVIDENCE_POLICY_TEMPLATES.md), and [`EVIDENCE_POLICY_EXAMPLES.md`](EVIDENCE_POLICY_EXAMPLES.md) for demo steps, guided review order, navigation/readiness review, operator packaging, local artifact verification, CI artifact consumption, release candidate dry-run review, install/run path selection, release-intent hard stops, package naming context, Postman import, scenario gallery review, comparison matrix review, replay mode review, review packet handoff, API contract trace review, explanation drill-down review, routing comparison, load-balancing cockpit review, lightweight proxy forwarding, proxy health/metrics demos, proxy retry/cooldown behavior, proxy demo stack startup, Java fixture launcher usage, proxy status UI review, strategy-specific proxy selected-upstream demos, CLI inputs, bundle export, manifest verification, evidence inventory, evidence catalog diffing, evidence handoff policies, packaged policy templates, example sender/receiver catalog pairs, safety guarantees, and JSON output.

Use [`REAL_BACKEND_PROXY_EXAMPLES.md`](REAL_BACKEND_PROXY_EXAMPLES.md) when adapting proxy mode to operator-controlled local or private HTTP services beyond the fixture launcher. It connects round-robin, weighted, failover, and retry/cooldown examples to `X-LoadBalancerPro-Upstream`, `X-LoadBalancerPro-Strategy`, `/proxy-status.html`, `/api/proxy/status`, CI artifact evidence, and the release-free checklist.
