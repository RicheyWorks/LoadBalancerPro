...
confirms its container is down, and applies the out-of-service `NoExecute` taint. It then requires that remediation to
force-remove the three exact stateless qualification pods from the API. The disposable cluster also pins immediate
EndpointSlice-triggered iptables
updates and a one-second kube-proxy cleanup sync; operators must review the equivalent setting or managed-ingress
