...
Configured upstreams with `healthy=false` are skipped before forwarding because the existing routing strategies consider only healthy candidates. If no healthy upstream is available, the proxy returns HTTP 503 with a deterministic JSON error:
...
Proxy mode does not dynamically mark an upstream unhealthy after a failed request. Use configuration or future health-check work to adjust upstream health.
...
`ReverseProxyDisabledTest`, `ReverseProxyControllerTest`, `ReverseProxyHealthAwareTest`, and `ReverseProxyFailureTest` use local in-process JDK `HttpServer` fixtures or unused loopback ports. They prove:
...
Good follow-up slices are active upstream health checks, a tiny local upstream fixture command for reviewer demos, strategy-specific proxy examples, and optional metrics for selected upstream, proxy failures, and forwarded status classes.
