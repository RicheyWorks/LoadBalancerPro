...
if ($DryRun) {
    ...
    Write-Host "Planned command: test-compile then EnterpriseLabProofToolsApplication --enterprise-lab-ownership-proof --enterprise-lab-ownership-proof-output=$OutputDir"
...
$run = Invoke-EnterpriseLabProofTool -Arguments @(
    ...
    "--enterprise-lab-ownership-proof-output=$OutputDir")
