...
class ReverseProxyDisabledTest {
    ...
    void proxyStatusReportsDisabledDefaults() throws Exception {
        mockMvc.perform(get("/api/proxy/status"))
                ...
                .andExpect(jsonPath("$.retry.budgetPercent").value(20))
                .andExpect(jsonPath("$.retry.backoffBaseMillis").value(50))
                .andExpect(jsonPath("$.retry.backoffMaxMillis").value(1000))
                .andExpect(jsonPath("$.retry.budgetPrimaryRequests").value(0))
                .andExpect(jsonPath("$.retry.budgetGrantedRetries").value(0))
                .andExpect(jsonPath("$.retry.budgetRejectedRetries").value(0))
                ...
                .andExpect(jsonPath("$.cooldown.slowStartDurationMillis").value(0))
