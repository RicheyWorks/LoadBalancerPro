...
For a top-level reviewer map that connects testing evidence to proxy demos, CI artifacts, release-free packaging, JavaFX desktop retirement, and package naming, see [`REVIEWER_TRUST_MAP.md`](REVIEWER_TRUST_MAP.md).
...
JavaFX desktop simulator and dependency retirement is documented in [`JAVAFX_OPTIONAL_UI.md`](JAVAFX_OPTIONAL_UI.md). The tested API, proxy, static browser, fixture launcher, artifact verification, and operator smoke paths remain JavaFX-free.

Real-backend copy/adapt examples are documented in [`REAL_BACKEND_PROXY_EXAMPLES.md`](REAL_BACKEND_PROXY_EXAMPLES.md). Static tests verify the guide, example profiles, loopback/private placeholders, status verification links, CI evidence links, release-free checklist links, default proxy-disabled behavior, and no `CloudManager` construction.
...
| Lightweight reverse proxy mode | `ReverseProxyDisabledTest`, `ReverseProxyForwardedHeadersTest`, `ReverseProxyLoadSheddingTest`, `ReverseProxyConsistentHashAffinityTest`, `ConsistentHashRingStrategyTest`, `ProxyRetryPolicyTest`, `ReverseProxyRetryBudgetSlowStartTest`, `LiveRoutingDecisionStoreTest`, `ReverseProxyLiveDecisionCaptureTest`, `ReverseProxyControllerTest`, `ReverseProxyHealthAwareTest`, `UpstreamHealthProberTest`, `ReverseProxyBackgroundHealthProberTest`, `ReverseProxyHealthMetricsTest`, `ReverseProxyFailureTest`, `ReverseProxyRetrySafetyTest`, `ReverseProxyRetryCooldownTest`, `ReverseProxyStrategyDemoLabTest`, `ProxyOperatorStatusPageTest`, `ProxyDemoStackUsabilityTest`, `ProxyDemoFixtureLauncherTest`, `OperatorPackagingPolishTest`, `OperatorDistributionSmokeKitTest`, `LocalArtifactVerificationTest`, `CiPackagedArtifactSmokeWorkflowTest`, `RealBackendProxyExamplesDocumentationTest` | Uses in-process loopback upstream fixtures and static safety checks to prove real local HTTP forwarding, query/body preservation, bounded FIFO capture of actual attempt decisions without request or target URL data, API-key/OAuth2 protection for decision reads, configured and background-health-aware skipping, dedicated daemon probe execution, rise/fall thresholds, stale-generation rejection, side-effect-free request/status/decision snapshot reads, anti-spoofing forwarding metadata for direct, trusted-proxy, and untrusted-proxy cases, validated static route header rewrites, process-local consistent-hash key stability and bounded remapping, signed cookie pinning/tamper fallback/unhealthy-target failover without key exposure, strict process-local global/per-upstream concurrency caps, fast overload 503 responses with `Retry-After`, priority-aware shedding with `CRITICAL` admitted last up to the hard cap, bounded adaptive-limit latency feedback, guarded limit reload, selected-upstream and strategy headers for `ROUND_ROBIN`, `WEIGHTED_ROUND_ROBIN`, and health-aware failover demos, forwarding/failure/retry/cooldown/status-class counters, deterministic percentage retry-budget capping, bounded exponential full-jitter backoff, non-idempotent no-retry defaults, cooldown skip/recovery with retained failure memory, new/recovered upstream slow-start weighting, upstream failure handling, static status UI safety, checked-in demo profile/script/docs safety, classpath availability of packaged static resources and demo profiles, Maven exec launcher docs, real-backend example profile safety and guide coverage, release-free distribution smoke docs/scripts, local artifact verification docs/scripts, CI packaged artifact smoke workflow safety, package naming documentation, JavaFX desktop-retirement note, Java fixture launcher parsing/output/health/echo behavior, default proxy disabled behavior, and no `CloudManager` construction. |
...
- Default tests now exercise a lightweight local reverse proxy path against loopback fixture servers, including strategy-specific selected-upstream behavior, active health checks, bounded retry-budget/backoff and cooldown/slow-start behavior, local in-memory metrics/status output, a read-only static proxy status UI, Java fixture launcher behavior, Maven exec/package docs, real-backend example profile and guide safety, release-free distribution smoke docs/scripts, local artifact verification docs/scripts, CI packaged artifact smoke workflow safety, namespace decision docs, and static safety checks for demo profiles/scripts/docs, but not a production gateway data plane.

## Next Credibility Step

The highest-value next testing move is long-running soak-style proxy checks that stay outside the default CI hot path.
