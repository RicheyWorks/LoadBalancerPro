...
Decision Explorer Phase 1 final handoff lives in [`docs/agent/DECISION_EXPLORER_PHASE1_FINAL_HANDOFF.md`](docs/agent/DECISION_EXPLORER_PHASE1_FINAL_HANDOFF.md). It records the Phase 1 PR sequence, merge evidence, implemented read-only and simulation-only behavior, verification posture, safety audit, remaining not-proven boundaries, and the recommended Decision Explorer Implementation Phase 2 campaign without adding production-readiness, certification, live-cloud, real-tenant, benchmark/load/stress, throughput/p95/p99, replay/export/storage, evidence-packet, or automation claims.
...
- Read-only Decision Replay Evidence Closure Packet lane: [`docs/ENTERPRISE_LAB_DECISION_REPLAY_EVIDENCE_CLOSURE_PACKET.md`](docs/ENTERPRISE_LAB_DECISION_REPLAY_EVIDENCE_CLOSURE_PACKET.md). It is in-response reviewer metadata only, not an export/share/download packet.
...
2. Run `mvn spring-boot:run`, then open [`http://localhost:8080/`](http://localhost:8080/), [`http://localhost:8080/routing-demo.html`](http://localhost:8080/routing-demo.html) as the legacy route for the Enterprise Lab routing cockpit, [`http://localhost:8080/enterprise-lab-reviewer.html`](http://localhost:8080/enterprise-lab-reviewer.html), [`http://localhost:8080/operator-evidence-dashboard.html`](http://localhost:8080/operator-evidence-dashboard.html), [`http://localhost:8080/adaptive-routing-scenarios.html`](http://localhost:8080/adaptive-routing-scenarios.html), [`http://localhost:8080/evidence-timeline.html`](http://localhost:8080/evidence-timeline.html), [`http://localhost:8080/evidence-export-packet.html`](http://localhost:8080/evidence-export-packet.html), [`http://localhost:8080/enterprise-lab.html`](http://localhost:8080/enterprise-lab.html), and [`http://localhost:8080/load-balancing-cockpit.html`](http://localhost:8080/load-balancing-cockpit.html).
...
- [`OPERATOR_PACKAGING.md`](docs/OPERATOR_PACKAGING.md) documents the Maven exec fixture launcher path, packaged-jar API/proxy commands, real-backend proxy example profiles, and JavaFX optionality.
...
If `LOADBALANCERPRO_API_KEY` is missing or blank, prod-profile `/api/**`, proxy-surface, OpenAPI, and Swagger requests fail closed with HTTP 401 except for the explicit public API exceptions. `/api/health` and Actuator health/info remain public for health validation and portfolio review; local/default profile API routes and Swagger/OpenAPI remain public for developer usability.
...
`POST /api/routing/compare` compares supported routing strategies against caller-provided candidate telemetry. Supported strategy IDs are `ROUND_ROBIN`, `TAIL_LATENCY_POWER_OF_TWO`, `WEIGHTED_LEAST_LOAD`, `WEIGHTED_LEAST_CONNECTIONS`, and `WEIGHTED_ROUND_ROBIN`. It is read-only and recommendation-only: it returns strategy results, explanations, additive Decision Vector evidence, Dominant Factor Analysis, Decision Delta Analysis, Decision Replay Snapshot metadata/fingerprints, Decision Replay Reconstruction Trace evidence steps/fingerprints, Decision Replay Capsule canonical evidence packaging/fingerprints, Decision Replay Readiness Checklist status, Decision Replay Evidence Source Map relationships, Decision Replay Evidence Boundary Summary metadata, Decision Replay Evidence Field Inventory metadata, Decision Evidence Null-Safety Summary metadata, Decision Evidence Status Rollup metadata, Decision Replay Evidence Lane Navigation Summary metadata, Decision Replay Evidence Lane Dependency Map metadata, Decision Replay Evidence Lane Reference Index metadata, Decision Replay Evidence Lane Dependency Summary metadata, Decision Replay Evidence Lane Consistency Summary metadata, Decision Replay Evidence Reviewer Snapshot metadata, Decision Replay Evidence Reviewer Guidance metadata, Decision Replay Evidence Reviewer Handoff Summary metadata, Decision Replay Evidence Reviewer Closure Summary metadata, top-level Decision Replay Evidence Closure Rollup metadata, and top-level Decision Replay Evidence Closure Checklist metadata where available. It does not call `CloudManager` or AWS, does not mutate cloud resources, does not mutate `LoadBalancer` allocation state, does not execute replay, does not perform what-if mutation, does not persist snapshot, trace, capsule, checklist, source-map, boundary-summary, field-inventory, null-safety-summary, status-rollup, lane-navigation, lane-dependency, lane-reference-index, lane-dependency-summary, lane-consistency-summary, reviewer-snapshot, reviewer-guidance, reviewer-handoff, reviewer-closure, reviewer-closure-rollup, or reviewer-closure-checklist state, or audit logs, does not generate a new source-map, boundary-summary, field-inventory, null-safety, status-rollup, lane-navigation, lane-dependency, lane-reference-index, lane-dependency-summary, lane-consistency-summary, reviewer-snapshot, reviewer-guidance, reviewer-handoff, reviewer-closure, reviewer-closure-rollup, or reviewer-closure-checklist fingerprint, does not add upload/share/download/export/PDF/ZIP behavior, and does not alter the capacity-aware or predictive allocation endpoints.

The same compare response can expose top-level `decisionReplayEvidenceReviewerClosurePacket` metadata derived only from returned closure summary, rollup, and checklist data. The packet is read-only in-response reviewer metadata, not an export/share/download packet, and it does not claim replay proof, scoring proof, correctness validation, production readiness, production certification, guaranteed replay, or production validation.
...
In local/default mode, OpenAPI UI is available at:
...
Supply `X-API-Key` for prod-like API-key review, or use local/default mode for unauthenticated generated-client inspection.
