...
If `LOADBALANCERPRO_API_KEY` is missing or blank, protected prod-profile API, proxy-surface, OpenAPI, and Swagger requests fail closed with HTTP 401. `/api/health` and Actuator health/info remain public for local validation and portfolio review; local/default profile Swagger/OpenAPI remains public for developer usability.
...
The profile does not require AWS credentials just to start the API, does not mutate AWS resources by default, and still protects API mutation/LASE observability endpoints, proxy forwarding/status surfaces, `/v3/api-docs`, and Swagger UI with `X-API-Key`. Treat it as a cloud-safety validation lane, not production mode.
...
In the local/default profile, `POST /api/routing/compare` works without an API key for local demos, and Swagger/OpenAPI remains public for developer usability. In the `prod` profile using `loadbalancerpro.auth.mode=api-key`, protected `POST` requests such as `POST /api/routing/compare` and `POST /api/evidence-training/scorecards/grade` require the configured `X-API-Key`. In the `cloud-sandbox` profile using API-key mode, routing comparison and scorecard grading have the same `X-API-Key` behavior and the profile remains dry-run by default. In OAuth2 mode, `POST /api/routing/**` requires the configured allocation role, which defaults to `operator`; other `/api/**` training onboarding routes require a valid authenticated token. `GET /api/lase/shadow` requires the configured `X-API-Key` in prod/cloud-sandbox API-key mode, or an `observer` or `operator` role in OAuth2 mode. `/proxy/**`, `GET /api/proxy/status`, `/v3/api-docs`, and Swagger UI require `X-API-Key` in prod/cloud-sandbox API-key mode and an authenticated token in OAuth2 mode. `/api/health` remains public.
