...
The first live-validation implementation primitive is still offline only. [`PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md`](PRIVATE_NETWORK_LIVE_VALIDATION_GATE.md) now documents `loadbalancerpro.proxy.private-network-live-validation.enabled=false` and `loadbalancerpro.proxy.private-network-live-validation.operator-approved=false` as default-off properties. `PrivateNetworkLiveValidationGate` requires both to be true alongside `loadbalancerpro.proxy.private-network-validation.enabled=true`, enabled proxy mode, and classifier-approved literal backend URLs before it returns an allowed decision. It does not send traffic, resolve DNS, probe reachability, discover hosts, scan ports, start Postman/smoke execution, or change routing behavior. Until a separate approved task adds a bounded traffic executor, private-network validation remains config validation, dry-run evidence, and an offline gate only.
