...
- I want to review the future private-network proxy profile before implementation: start with [`PRIVATE_NETWORK_PROXY_PROFILE_PLAN.md`](PRIVATE_NETWORK_PROXY_PROFILE_PLAN.md).
...
| Is the future private-network proxy profile bounded before implementation? | Design plan and static guard tests | [`PRIVATE_NETWORK_PROXY_PROFILE_PLAN.md`](PRIVATE_NETWORK_PROXY_PROFILE_PLAN.md) | Explicit operator-provided backend URLs, local/private-network allowlist model, redacted ignored evidence plan | The future profile path forbids scanning, discovery, persistence, service installation, scheduled tasks, native tooling, secret persistence, public internet validation, and `release-downloads/` mutation before any runtime work | Runtime implementation, private-network live execution, or public exposure approval |
