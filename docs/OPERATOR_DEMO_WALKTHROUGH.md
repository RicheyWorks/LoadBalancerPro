...
9. Copy curl snippets, the selected scenario payload, raw responses, cockpit summary, scenario summary, drill-down summary, explanation curl snippets, or operator rationale.
10. Reset the cockpit before replaying it.
...
The cockpit does not add a new API contract. It reuses existing calculation and recommendation routes, marks unsupported sections as `Not available in current API`, and does not fabricate allocation, routing, load-shedding, or remediation behavior. The explanation drill-down labels client-side supporting math as derived from visible request/response fields when exact internal scores or thresholds are not exposed by the current API. The matching Postman flows are the `Unified Load-Balancing Cockpit`, `Operator Scenario Gallery`, and `Operator Explanation Drill-Down` folders. See [`POSTMAN_LOAD_BALANCING_COCKPIT.md`](POSTMAN_LOAD_BALANCING_COCKPIT.md), [`OPERATOR_SCENARIO_GALLERY.md`](OPERATOR_SCENARIO_GALLERY.md), and [`OPERATOR_EXPLANATION_DRILLDOWN.md`](OPERATOR_EXPLANATION_DRILLDOWN.md) for request order, sample bodies, expected response shape, scenario comparison, drill-down behavior, and safety limits.
...
The evidence browser cockpit is a tiny static GUI-facing surface served by the existing Spring Boot app at `/evidence-training-demo.html`. It reuses the existing evidence training API endpoints and deterministic embedded sample payloads. The routing decision demo at `/routing-demo.html` is the matching static GUI-facing surface for the existing request-level routing comparison API. The unified load-balancing cockpit at `/load-balancing-cockpit.html` ties allocation, routing, overload/load-shedding, remediation-hint, scenario-gallery, and explanation drill-down views together with existing API responses. These pages add no frontend framework, external dependency, external script, CDN, browser automation dependency, or controller.
