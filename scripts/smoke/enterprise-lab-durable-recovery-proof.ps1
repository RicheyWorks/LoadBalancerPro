...
if ($DryRun) {
    ...
    Write-Host "Planned command: test-compile then EnterpriseLabProofToolsApplication --enterprise-lab-durable-recovery-proof --enterprise-lab-durable-recovery-output=$OutputDir"
...
$run = Invoke-EnterpriseLabProofTool -Arguments @(
    ...
    "--enterprise-lab-durable-recovery-output=$OutputDir")
