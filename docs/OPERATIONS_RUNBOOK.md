...
In local/default mode, those evidence-training routes remain public for onboarding convenience. In prod/cloud-sandbox API-key mode, all `/api/evidence-training/**` routes require `X-API-Key` because `/api/**` is deny-by-default except for `GET /api/health` and unauthenticated `OPTIONS` preflight requests.
