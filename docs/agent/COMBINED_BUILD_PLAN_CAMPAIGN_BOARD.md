...
- `MAIN_GREEN` implementation slots: 6 / 49.
...
- Active implementation slot: `L-0.2`; explorer candidate, strategy, and response size caps are locally green on
  branch `codex/l-0-2-explorer-size-caps` and await one exact-head PR plus mandatory remote gates.
...
| 7 | L-0.2 | L-0.2 | Explorer size caps | SEC-DEFAULT-DENY | LOCAL_GREEN |
