...
`POST /api/routing/compare` compares supported routing strategies against caller-provided candidate telemetry. Supported strategy IDs are `ROUND_ROBIN`, `TAIL_LATENCY_POWER_OF_TWO`, `WEIGHTED_LEAST_LOAD`, `WEIGHTED_LEAST_CONNECTIONS`, and `WEIGHTED_ROUND_ROBIN`. It is read-only and recommendation-only: it returns strategy results, explanations, additive Decision Vector evidence, Dominant Factor Analysis, Decision Delta Analysis, Decision Replay Snapshot metadata/fingerprints, Decision Replay Reconstruction Trace evidence steps/fingerprints, Decision Replay Capsule canonical evidence packaging/fingerprints, Decision Replay Readiness Checklist status, Decision Replay Evidence Source Map relationships, Decision Replay Evidence Boundary Summary metadata, and Decision Replay Evidence Field Inventory metadata where available. It does not call `CloudManager` or AWS, does not mutate cloud resources, does not mutate `LoadBalancer` allocation state, does not execute replay, does not perform what-if mutation, does not persist snapshot, trace, capsule, checklist, source-map, boundary-summary, field-inventory state, or audit logs, does not generate a new source-map, boundary-summary, or field-inventory fingerprint, does not add upload/share/download/export/PDF/ZIP behavior, and does not alter the capacity-aware or predictive allocation endpoints.
