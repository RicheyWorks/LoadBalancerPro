...
| Lightweight reverse proxy mode | `ReverseProxyDisabledTest`, `ReverseProxyControllerTest`, `ReverseProxyHealthAwareTest`, `ReverseProxyHealthMetricsTest`, `ReverseProxyFailureTest`, `ReverseProxyRetrySafetyTest`, `ReverseProxyRetryCooldownTest`, `ReverseProxyStrategyDemoLabTest`, `ProxyOperatorStatusPageTest`, `ProxyDemoStackUsabilityTest`, `ProxyDemoFixtureLauncherTest`, `OperatorPackagingPolishTest` | Uses in-process loopback upstream fixtures and static safety checks to prove real local HTTP forwarding, query/body preservation, configured and active health-aware skipping, selected-upstream and strategy headers for `ROUND_ROBIN`, `WEIGHTED_ROUND_ROBIN`, and health-aware failover demos, read-only status output, forwarding/failure/retry/cooldown/status-class counters, bounded GET retry behavior, non-idempotent no-retry defaults, cooldown skip/recovery, upstream failure handling, static status UI safety, checked-in demo profile/script/docs safety, Maven exec launcher docs, real-backend example profile safety, package naming documentation, JavaFX optional note, Java fixture launcher parsing/output/health/echo behavior, default proxy disabled behavior, and no `CloudManager` construction. |
...
- Default tests now exercise a lightweight local reverse proxy path against loopback fixture servers, including strategy-specific selected-upstream behavior, active health checks, bounded retry/cooldown behavior, local in-memory metrics/status output, a read-only static proxy status UI, Java fixture launcher behavior, Maven exec/package docs, real-backend example profile safety, namespace decision docs, and static safety checks for demo profiles/scripts/docs, but not a production gateway data plane.
