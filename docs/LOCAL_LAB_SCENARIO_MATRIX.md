...
It is planning-only. It does not implement Docker Compose files, scripts, fake backend nodes, k6 scenario files, expanded Bruno collections beyond the optional local-lab skeleton, expanded Toxiproxy fault execution beyond the optional local-lab config skeleton, Prometheus/Grafana dashboards, telemetry ingestion, replay execution, report generation, storage/persistence, filesystem-writing behavior, export/upload/download/PDF/ZIP behavior, routing/scoring/strategy/proxy/API behavior changes, or production validation.
...
The current local-lab tool lane now has one optional local-lab k6 smoke script skeleton at [`../lab/k6/local-lab-smoke.js`](../lab/k6/local-lab-smoke.js), one optional local-lab Bruno collection skeleton at [`../lab/bruno/local-lab-smoke/`](../lab/bruno/local-lab-smoke/), one optional local-lab Toxiproxy config skeleton at [`../lab/toxiproxy/local-lab-toxiproxy.json`](../lab/toxiproxy/local-lab-toxiproxy.json), and one optional local-lab Docker Compose skeleton at [`../lab/docker-compose/local-lab-compose.yml`](../lab/docker-compose/local-lab-compose.yml). [`LOCAL_LAB_MANUAL_TOOLING_INDEX.md`](LOCAL_LAB_MANUAL_TOOLING_INDEX.md) is the unified docs/test-only reviewer index for those manual tool skeletons, [`LOCAL_LAB_MANUAL_TOOLING_RUNBOOK.md`](LOCAL_LAB_MANUAL_TOOLING_RUNBOOK.md) is the companion manual reviewer/operator runbook for inspection-only review and optional local-only manual use, [`LOCAL_LAB_DOCKER_COMPOSE_BOUNDARY_DESIGN.md`](LOCAL_LAB_DOCKER_COMPOSE_BOUNDARY_DESIGN.md) is future-only Docker Compose boundary design, [`LOCAL_LAB_DOCKER_COMPOSE_SKELETON.md`](LOCAL_LAB_DOCKER_COMPOSE_SKELETON.md) documents the current optional Compose skeleton, [`LOCAL_LAB_DOCKER_COMPOSE_MANUAL_RUNBOOK.md`](LOCAL_LAB_DOCKER_COMPOSE_MANUAL_RUNBOOK.md) gives the Compose-specific manual checklist, [`LOCAL_LAB_DOCKER_COMPOSE_APP_SERVICE_BOUNDARY_DESIGN.md`](LOCAL_LAB_DOCKER_COMPOSE_APP_SERVICE_BOUNDARY_DESIGN.md) documents future-only app-service prerequisites and stop conditions, and [`LOCAL_LAB_DOCKER_COMPOSE_READINESS_GATE.md`](LOCAL_LAB_DOCKER_COMPOSE_READINESS_GATE.md) documents the future-change checklist before editing Compose or adding services. See [`LOCAL_LAB_K6_SMOKE_SCRIPT.md`](LOCAL_LAB_K6_SMOKE_SCRIPT.md), [`LOCAL_LAB_BRUNO_COLLECTION.md`](LOCAL_LAB_BRUNO_COLLECTION.md), [`LOCAL_LAB_TOXIPROXY_CONFIG.md`](LOCAL_LAB_TOXIPROXY_CONFIG.md), [`LOCAL_LAB_BRUNO_COLLECTION_DESIGN.md`](LOCAL_LAB_BRUNO_COLLECTION_DESIGN.md), and [`LOCAL_LAB_TOXIPROXY_FAULT_MODEL_DESIGN.md`](LOCAL_LAB_TOXIPROXY_FAULT_MODEL_DESIGN.md). The Bruno skeleton is optional and manual, not CI-gated, not Dockerized, not Toxiproxy integration, not k6 execution, and must target local/lab-owned loopback endpoints by default. The Toxiproxy skeleton is optional, manual-only, not CI-gated, not wired into the application, not wired into Maven, not wired into k6 execution, not wired into Bruno execution, does not start the application, and must target local/lab-owned loopback endpoints by default. The Compose skeleton is optional, manual-only, local-lab-only, loopback-bound, not CI-gated, not wired into Maven, not production Docker packaging, not production runtime behavior, not wired into k6 execution, not wired into Bruno execution, and not wired into automated execution. The Compose manual runbook adds no new services, no app service, no k6 runner service, no Bruno runner service, no CI-gating, no Maven wiring, no production Docker packaging, no production runtime behavior, and no automated execution. The app-service boundary design adds no app service, no Compose behavior changes, no Docker packaging changes, no CI-gating, no Maven wiring, and no production runtime behavior. The Compose readiness gate adds no Compose behavior changes, no app service, no new Compose services, no k6 runner service, no Bruno runner service, no CI-gating, no Maven wiring, no production Docker packaging, and no production runtime behavior. It does not add app container orchestration, k6 runner services, Bruno runner services, production endpoints, production listeners, routing/scoring/strategy/proxy/API behavior changes, replay execution, evidence/report generation, storage/export behavior, load/stress/benchmark testing, throughput evidence, or p95/p99 evidence.
...
The end-of-day Compose handoff and post-app-service Compose handoff update are recorded in [`LOCAL_LAB_PROGRESS_HANDOFF.md`](LOCAL_LAB_PROGRESS_HANDOFF.md) and [`LOCAL_LAB_NEXT_STEPS_BOUNDARY.md`](LOCAL_LAB_NEXT_STEPS_BOUNDARY.md). They summarize the merged local-lab sequence through the gated app-service skeleton, record that `app-under-test` now exists in local-lab Compose as optional/manual/local-lab-only, keep k6 and Bruno manual and separate, keep Toxiproxy manual/local-only, confirm Compose is not CI-gated and not Maven-wired, confirm no production Docker packaging has been added, and point any future Compose change back to the Compose readiness gate and app-service preflight checklist.
...
PR #284 is now the current local-lab Compose baseline. This scenario-matrix update is docs/test-only and does not change `lab/docker-compose/local-lab-compose.yml`, Dockerfiles, Maven, CI, runtime resources, k6, Bruno, Toxiproxy behavior, scripts, production Compose, production Docker packaging, or production runtime behavior.
